...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    /**
     * kafkaProducer初始化都是在这个方法里面实现的，这个方法有很多参数，也有很多重要的类
     * @param config 我们传入的一些必要参数，以及各种默认参数都配置在ProducerConfig当中
     * @param keySerializer  序列化
     * @param valueSerializer  序列化
     * @param metadata  元数据信息
     * @param kafkaClient  客户端对象
     * @param interceptors  拦截器
     * @param time  数据发送的默认系统时间
     */
    ...
    KafkaProducer(ProducerConfig config,
                  ...
                  Time time) {
        try {
            //通过传入的参数给全局变量producerConfig进行赋值
            this.producerConfig = config;
            //设置当前系统时间
            this.time = time;

            //在kafka新版本当中，生产者可以配置事务id，用于保证数据生产的事务性，如果没有提供事务id，那么生产者仅限于幂等性来保证数据的exactly  once语义
            String transactionalId = config.getString(ProducerConfig.TRANSACTIONAL_ID_CONFIG);

            this.clientId = config.getString(ProducerConfig.CLIENT_ID_CONFIG);

            //LogContext主要是用于在没有添加groupID的时候，将消息进一步包装，每个消费者添加一个唯一标识
            ...
            //开始设置消息的分区器，分区器决定了消息使用哪种方式发送到指定topic的哪个分区里面去
            ...
            //设置拦截器，拦截器给准备发送出去的消息先添加了一道拦截，可以实现对消息的过滤
            ...
            //todo:  指的是缓存大小
            //默认值是32M，这里设置的是内存缓冲区的大小默认为32M，这个内存大小主要是用于发送数据的缓冲区大小
            //生产者发送数据出去之前，都需要先对数据在本地进行缓冲，这里就是本地缓冲区的大小，默认是32M，
            //如果发送速度比较快，缓冲区到服务器broker比较慢，那么就会把这个缓冲区填满，缓冲区填满了生产者线程就会阻塞
            //阻塞时间由max.block.ms 这个配置选项决定，默认是60s，实际工作当中，这个缓冲区大小，一般都跟producer端配置的内存大小保持一致
            this.totalMemorySize = config.getLong(ProducerConfig.BUFFER_MEMORY_CONFIG);
            //设置数据的压缩方式，很明显数据发送到服务端之前，可以先在本地缓冲区进行压缩一下，减少数据体积，减少网络占用
            //压缩方式的值可以是  node,gzip,snappy,lz4,zstd等各种方式，压缩是对发送给broker的一批数据进行压缩，
            //用时间换空间的思想，压缩更加消耗CPU，消耗压缩时间，但是发送数据的吞吐量更大了。计算机里面到处都是各种时间换空间，空间换时间的思想。
            this.compressionType = CompressionType.forName(config.getString(ProducerConfig.COMPRESSION_TYPE_CONFIG));
            //这个阻塞时间，决定了生产者在发送数据之前，这个超时参数限制了等待元数据获取和缓冲区分配的总时间，用户提供的序列化程序和分区程序中的阻塞不算在这个时间里面
            //说白了，我的数据发送到哪一个分区需要先拉取元数据信息，然后通过元数据当中的分区数，以及分区方法决定数据进入哪一个分区
            //如果元数据拉取超时，本地缓冲区满了阻塞，都会等待这么长时间
            ...
            //这里初始化了一个核心组件叫做RecordAccumulator，这里主要是用这个对象来对消息进行本地缓存，说白了就是为了攒一批消息一起发送出去
            ...
            if (metadata != null) {
                ...
                this.metadata = new ProducerMetadata(retryBackoffMs,
                        ...
                        Time.SYSTEM);
                //元数据信息对象调用了bootstrap方法，进入查看bootstrap方法
    ...
    Sender newSender(LogContext logContext, KafkaClient kafkaClient, ProducerMetadata metadata) {
        // maxInflightRequests 生产者发送消息给服务端，具体到服务端某个leader Partition去接收消息，但是leader Partition有可能宕机下线了，那么生产者怎么办？？
        //这里有两种选项，1、发送不过去了就不发了，客户端阻塞，不给这个partition发送消息了
        //2、继续发送，如果发送maxInflightRequests  次没响应就不发了，例如maxInflightRequests取值为5，那么就是发送5次过去没响应就不发了，那么这里就存在消息乱序的问题了
        // A给B发送消息，由于网络原因，B没有收到，A继续发送了五次，等到B的网络好了，那么五次消息当中消息还会按照顺序发送的方式到达B嘛？？？如果不能顺序到达，那么是不是就出现了消息乱序的问题了
        //大家都说kafka是分区内部有序的，源码当中maxInflightRequests默认取值是5，说明分区内部也有可能乱序
         ...
         * 注意：这里为什么发送缓冲区比接收缓冲区大，因为发送缓冲区设置大一点，可以将多批次消息合并成为一个包，进行沾包处理，提高发送效率
         */
    ...
    /**
     * 通过异步的方式来进行消息的方法，并且调用回调函数来通知生产者消息是否发送成功 ，这个方法一旦调用，消息写入到本地缓冲区，就会立马返回，这样可以提高消息发送的效率
     * 可以并行的一起发送消息，都写入到本地缓冲区，而且不需要等待服务器的响应  。最终执行完了，这个方法的返回结果是这个数据写入到kafka服务器的元数据信息，例如这个数据的分区号，这个数据的offset对应的值等
     * 发送数据还可以带上数据的时间戳，如果我们给数据指定了CREATE_TIME这个属性，那么就使用我们自己指定的时间戳的值，如果没有指定，那么就使用数据的发送出来的时间，如果启用了LOG_APPEND_TIME,
     * 那么就使用kafka集群的服务器时间作为每条数据的时间戳
    ...
    private int partition(ProducerRecord<K, V> record, byte[] serializedKey, byte[] serializedValue, Cluster cluster) {
        //如果数据指定有分区号，直接使用数据指定的分区号，否则进行计算分区号
