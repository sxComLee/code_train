...
public class NetworkClient implements KafkaClient {
    ...
    public boolean isReady(Node node, long now) {
        ...
        // priority
        //如果需要更新元数据信息，那么需要把更新元数据信息的请求等级提到最高优先级，先更新元数据，才能发送数据出去
    ...
    public List<ClientResponse> poll(long timeout, long now) {
        ...
        if (!abortedSends.isEmpty()) {
            ...
            //判断如果版本不支持，或者断开连接，优先处理这些不支持的发送请求
    ...
    class DefaultMetadataUpdater implements MetadataUpdater {
        ...
        public long maybeUpdate(long now) {
            ...
            //继续调用了maybeUpdate方法，查看maybeUpdate方法具体实现如下
        ...
        public void handleSuccessfulResponse(RequestHeader requestHeader, long now, MetadataResponse response) {
            ...
            //将响应的response进行处理成为一个集合，确定哪些分区没有监听器
            ...
            //如果没有获取到broker，元数据拉取失败
            if (response.brokers().isEmpty()) {
                log.trace("Ignoring empty metadata response with correlation id {}.", requestHeader.correlationId());
                this.metadata.failedUpdate(now);
            } else {
                //todo:元数据获取到了，真正的去更新元数据信息
        ...
        private long maybeUpdate(long now, Node node) {
            ...
            if (canSendRequest(nodeConnectionId, now)) {
                //这里来判断是全量拉取数据还是增量拉取数据
                Metadata.MetadataRequestAndVersion requestAndVersion = metadata.newMetadataRequestAndVersion(now);
                MetadataRequest.Builder metadataRequest = requestAndVersion.requestBuilder;
                log.debug("Sending metadata request {} to node {}", metadataRequest, node);
                //todo:创建了一个拉取元数据的请求，所有的元数据的获取网络请求都在这里
