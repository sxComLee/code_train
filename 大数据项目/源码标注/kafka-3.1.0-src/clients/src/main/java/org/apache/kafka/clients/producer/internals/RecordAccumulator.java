...
public final class RecordAccumulator {
    ...
    public RecordAppendResult append(TopicPartition tp,
        ...
        //这里通过jdk8当中的concurrent并发包下面的AtomicInteger原子类来记录添加数据的字节数组大小
        //这里还使用到了jdk8当中的UnSafe类，这个类提供了硬件级别的原子操作，我们都知道java不能直接调用硬件级别的资源，但是UnSafe这个类可以直接调用硬件的资源
        //便于在java层面可以实现原本需要在底层（C层）才能完成的操作。主要包括内存的申请/访问/释放、atomic/volatile等操作。该类设计成单例，并作了访问控制
        //说白了kafka为啥这么快，这里就是一个突出表现，需要使用内存，直接自己去拿硬件内存来用，没有依靠JVM的堆内存，需要反复的回收和释放，避免了频繁的GC
        appendsInProgress.incrementAndGet();
        ByteBuffer buffer = null;
        if (headers == null) headers = Record.EMPTY_HEADERS;
        try {
            // check if we have an in-progress batch
            //判断我们当前是否有正在往里面添加的批次数据，如果有，直接添加进去，没有就创建新的批次
            /**
             *    todo: 第一步：获取消息所属队列
             *    如果属于已经存在的队列，直接往该队列里面写入
             *    如果队列不存在，那么自己创建一个新的队列
             *    第一次肯定不存在队列
             */
            Deque<ProducerBatch> dq = getOrCreateDeque(tp);
            //todo:使用分段锁的方式，避免多线程问题
            synchronized (dq) {
                if (closed)
                    throw new KafkaException("Producer closed while send in progress");
                /**
                 * todo:步骤二 尝试向已有的队列里面添加批次数据
            ...
            //使用分段锁的方式，只上锁这一段代码，没有上锁整个方法，效率更高
            synchronized (dq) {
    ...
    private RecordAppendResult tryAppend(long timestamp, byte[] key, byte[] value, Header[] headers,
        ...
        if (last != null) {
            //通过调用ProducerBatch对象的tryAppend方法，向RecordAccumulator当中进行写入数据，说白了就是向本地缓冲区中根据数据不同的分区号，写入到不同的本地缓冲区文件当中去
            FutureRecordMetadata future = last.tryAppend(timestamp, key, value, headers, callback, nowMs);
            if (future == null)
                //如果写入返回值对象为空，那么就直接关闭当前批次的写入
                last.closeForRecordAppends();
            else
                //如果当前批次写满了 那么就创建一个新的写入批次
    ...
    public ReadyCheckResult ready(Cluster cluster, long nowMs) {
        //定义set集合，用于存放那些准备好的服务器
        Set<Node> readyNodes = new HashSet<>();
        long nextReadyCheckDelayMs = Long.MAX_VALUE;
        Set<String> unknownLeaderTopics = new HashSet<>();
        //todo:waters队列里面有数据，说明内存池中的内存不够了
        ...
        for (Map.Entry<TopicPartition, Deque<ProducerBatch>> entry : this.batches.entrySet()) {
            ...
            synchronized (deque) {
                ...
                //如果有大量的批次数据，可能就要往分区里面写入大量的数据，先对本次是否存在批次数据进行检测，避免所有的批次数据写入都要检测，更加慢了

                //获取队列第一个批次
                ProducerBatch batch = deque.peekFirst();
                if (batch != null) {
                    //获取到topic以及对应的partition
                    ...
                    if (leader == null) {
                         ...
                         *  backingOff 应该为false
                         *
                         *  最多能等多久timeToWaitMs=lingerMs 默认就是0 ，表示不需要等待
    ...
    private Deque<ProducerBatch> getOrCreateDeque(TopicPartition tp) {
        //通过当前对象调用batches获取到了ConcurrentMap这个对象
        //todo:在RecordAccumulator这个对象初始化的时候，初始化了一个属性  this.batches = new CopyOnWriteMap<>();
        //这个CopyOnWriteMap是kafka自己封装的一个类似Map集合的类
