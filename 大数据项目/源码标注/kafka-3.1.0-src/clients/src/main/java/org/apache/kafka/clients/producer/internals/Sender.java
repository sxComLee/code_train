...
public class Sender implements Runnable {
            ...
            if (partitionInFlightBatches != null) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (batch.hasReachedDeliveryTimeout(accumulator.getDeliveryTimeoutMs(), now)) {
                        ...
                        if (!batch.isDone()) {
                            //所有过期的batch数据都添加到expiredBatches集合当中去了
    ...
    public void run() {
        ...
        // sender线程就是一个死循环，一直在运行，一直在调用runOnce方法
        while (running) {
            try {
                runOnce();
            } catch (Exception e) {
    ...
    void runOnce() {
        if (transactionManager != null) {
            try {
                ...
                if (transactionManager.hasFatalError()) {
                    ...
                    //todo:通过客户端对象，调用poll方法去拉取元数据，真正的发送网络请求都是在这个方法里面，后续我们
                    //专门讲网络通信这一块儿再回来查看这块儿源码
        ...
        //使用ctrl + alt + B进入到方法的具体实现中去查看具体代码
    ...
    private long sendProducerData(long now) {
        ...
        //将待发送的batch添加到Sender的inFlightBatches中，
    ...
    private void handleProduceResponse(ClientResponse response, Map<TopicPartition, ProducerBatch> batches, long now) {
        ...
        if (response.wasDisconnected()) {
            ...
            if (response.hasResponse()) {
                ...
                //如果ack方式设置为0 ，不需要确定消息是否保存成功，那么就直接运行这段即可
                for (ProducerBatch batch : batches.values()) {
    ...
    private void sendProduceRequests(Map<Integer, List<ProducerBatch>> collated, long now) {
        //这里循环遍历每一个批次，每一个批次的数据循环出来之后，又调用了sendProduceRequest这个方法，注意这个方法跟sendProduceRequests是不一样的，这两个方法
        //没有关系，不是递归，也不是重载
    ...
    private void sendProduceRequest(long now, int destination, short acks, int timeout, List<ProducerBatch> batches) {
        ...
        //比那里ProducerBatch集合，整理成ProducerBatch和MemoryRecords
        ...
        //构建ClientRequest对象，这个才是真正准备发送数据的对象
        ClientRequest clientRequest = client.newClientRequest(nodeId, requestBuilder, now, acks != 0,
                requestTimeoutMs, callback);
        //调用send方法，发送数据,这里的client其实就是KafkaClient，注意在，
        // 使用crtl +  Alt + B进入到实现类方法中去查看具体实现，具体实现是NetworkClient
        //继续查看下去，就涉及到网络操作了，网络操作比较复杂，我们留给后面专门分析网络操作
