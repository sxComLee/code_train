...
public class MemoryRecordsBuilder implements AutoCloseable {
    ...
    private void appendWithOffset(long offset, boolean isControlRecord, long timestamp, ByteBuffer key,
        ...
        try {
            ...
            //这里判断 magicNumber的值与RecordBatch的默认值，确定是否需要将magicNumber添加到数据当中去
            //这个magicNumber一般没啥用，基本上发送数据都没有带上，所以直接走appendDefaultRecord这个方法
            if (magic > RecordBatch.MAGIC_VALUE_V1) {
    ...
    private void recordWritten(long offset, long timestamp, int size) {
        //如果数据条数太多，直接抛出异常，这个判断基本上没啥用，你一批次数据基本上写不了这么多条数据
        ...
        //这个方法就直接将numRecords的值自增1，来一条数据加1
        numRecords += 1;
        //未压缩的数据量大小，也加上数据的字节数组大小
