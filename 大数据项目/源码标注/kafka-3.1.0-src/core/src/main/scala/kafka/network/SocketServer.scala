...
class SocketServer(val config: KafkaConfig,
   ...
   * 启动SocketServer服务端，并且创建接收器以及处理器，在SocketServer开始启动到完全启动完成这一阶段，接收器就开始
   * 通过绑定的端口开始接收请求，接收器与处理器如果在startProcessingRequest标志位true的时候，表示启动完成，
   * 如果这个标志位false，接收器和处理器只有在调用了startProcessingRequest方法之后，才会启动。
   * 如果接收器和处理器延迟启动，那么就主要是用于延迟处理客户端的链接请求。
   * 从main方法当进来，第一次startProcessingRequests默认标志位为false
   */
   ...
   *
   *    启动Processor来处理客户端的请求，并且启动连接，用于接收客户端的请求，说白了就是创建Acceptor线程呗
   *    其中启动Processor线程 之前就需要保证所有的集群元数据信息都已经获取到了
   *
   */
  def startProcessingRequests(authorizerFutures: Map[Endpoint, CompletableFuture[Void]] = Map.empty): Unit = {
    info("Starting socket server acceptors and processors")
    this.synchronized {
      //第一次startedProcessingRequests默认为false，刚好进入if判断
      if (!startedProcessingRequests) {
        //启动controller的请求接收以及请求处理线程
  ...
  private def createDataPlaneAcceptorsAndProcessors(dataProcessorsPerListener: Int,
    ...
    endpoints.foreach { endpoint =>
      ...
      //创建数据处理的accecptor 在这里里面定义了socket发送和接收缓冲区的大小
      val dataPlaneAcceptor = createAcceptor(endpoint, DataPlaneMetricPrefix)
      //同样的操作将acceptor添加到了processor里面去了
  ...
  private def createControlPlaneAcceptorAndProcessor(endpointOpt: Option[EndPoint]): Unit = {
    endpointOpt.foreach { endpoint =>
      connectionQuotas.addListener(config, endpoint.listenerName)
      //todo:创建了一个accoptor线程
      val controlPlaneAcceptor = createAcceptor(endpoint, ControlPlaneMetricPrefix)
      //todo：创建了一个processor线程
      ...
      //todo：将processor创建好了之后，添加到了一个ArrayBuffer缓冲队列里面去了
      val listenerProcessors = new ArrayBuffer[Processor]()
      listenerProcessors += controlPlaneProcessor
      //todo：循环遍历每一个processor
      controlPlaneRequestChannelOpt.foreach(_.addProcessor(controlPlaneProcessor))
      nextProcessorId += 1
      //todo：将processor添加到acceptor里面去了
...
private[kafka] class Acceptor(val endPoint: EndPoint,
  ...
  def run(): Unit = {
    ...
    startupComplete()
    try {
      //只要服务端启动在运行，就形成了一个死循环，一直接收客户端请求
      while (isRunning) {
        try {
          //不断的接收客户端的请求
  ...
  private def accept(key: SelectionKey): Option[SocketChannel] = {
    ...
    try {
      ...
      //将所有的请求都添加到ServerSocketChannel当中去了
      Some(socketChannel)
    } catch {
...
private[kafka] class Processor(val id: Int,
  ...
  private def processCompletedSends(): Unit = {
    ...
    selector.completedSends.forEach { send =>
      try {
        //已经响应完成的请求从inflightResponses当中移除
  ...
  private def configureNewConnections(): Unit = {
    ...
    while (connectionsProcessed < connectionQueueSize && !newConnections.isEmpty) {
      //todo:不断的获取连接队列当中的SocketChannel
