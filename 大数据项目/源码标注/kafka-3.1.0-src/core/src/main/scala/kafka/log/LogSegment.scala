...
class LogSegment private[log] (val log: FileRecords,
 ...
 //   size > rollParams.maxSegmentBytes - rollParams.messagesSize || (size > 0 && reachedRollMs) || offsetIndex.isFull || timeIndex.isFull || !canConvertToRelativeOffset(rollParams.maxOffsetInMessages)
  ...
  def append(largestOffset: Long,
    ...
    if (records.sizeInBytes > 0) {
      ...
      // todo:确保输入参数最大位移值是合法的
      ensureOffsetInRange(largestOffset)

      //todo：日志文件追加到leader主分区当中去
      // append the messages
      val appendedBytes = log.append(records)
      trace(s"Appended $appendedBytes to ${log.file} at end offset $largestOffset")
      // Update the in memory max timestamp and corresponding offset.
      //todo： 更新最大的timestamp和对应的offset
  ...
  def read(startOffset: Long,
    ...
    //最后将获取到的数据封装成为FetchDataInfo进行返回
