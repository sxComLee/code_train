...
class ReplicaManager(val config: KafkaConfig,
  ...
  def appendRecords(timeout: Long,   // 请求处理超时时间
    ...
    if (isValidRequiredAcks(requiredAcks)) {
      ...
      //todo：根据本地写入日志返回的结果，封装状态响应给客户端，告诉客户端数据是否写入成功
      val produceStatus = localProduceResults.map { case (topicPartition, result) =>
        topicPartition -> ProducePartitionStatus(
          //todo: 设置下一条消息的offset值
          result.info.lastOffset + 1, // required offset
          //todo：构建PartitionResponse对象，封装每个分区数据写入的响应结果
      ...
      //将数据写入本地的响应结果添加到队列里面去，便于并行的处理响应结果
      actionQueue.add {
        () =>
  ...
  private def appendToLocalLog(internalTopicsAllowed: Boolean,
    ...
    entriesPerPartition.map { case (topicPartition, records) =>
      ...
      if (Topic.isInternal(topicPartition.topic) && !internalTopicsAllowed) {
        ...
        try {
          //todo:获取分区信息，或者获取不到就抛出异常
          val partition = getPartitionOrException(topicPartition)
          //todo：重点来了，将消息添加到leader主分区里面去
          val info = partition.appendRecordsToLeader(records, origin, requiredAcks, requestLocal)
          val numAppendedMessages = info.numMessages

          // update stats for successfully appended bytes and messages as bytesInRate and messageInRate
          //todo:更新写入数据的额状态以及写入数据的速率
          ...
          // it is supposed to indicate un-expected failures of a broker in handling a produce request
              //todo:找不到topic分区对应的partition对象，直接抛出异常
          case e@ (_: UnknownTopicOrPartitionException |
                   _: NotLeaderOrFollowerException |
  ...
  def fetchMessages(timeout: Long,
    ...
    //判断请求是从consumer来拉取数据还是从follower拉取数据
  ...
  def becomeLeaderOrFollower(correlationId: Int,
    ...
    //记录broker成为leader或者follower的起始时间
    val startMs = time.milliseconds()
    //在broker状态变更的时候，需要同步加锁
    replicaStateChangeLock synchronized {
      //获取到controller节点id，
      ...
      //获取该请求处理最后的response响应
      val response = {
        //判断如果controller节点的Epoch状态值大于该请求的controller  的Epoch值，直接抛出异常
        if (leaderAndIsrRequest.controllerEpoch < controllerEpoch) {
          ...
          //todo:第一步：循环遍历每一个分区状态，如果分区不存在，先创建分区
          requestPartitionStates.foreach { partitionState =>
            ...
            //todo:第二步：检查topicID以及分区的Leader的epoch值
            partitionOpt.foreach { partition =>
              ...
              if (!hasConsistentTopicId(requestTopicId, logTopicId)) {
                ...
                if (partitionState.replicas.contains(localBrokerId)) {
                  ...
                  //todo：这里判断partitionState对应的leader值，如果等于localBrokerId那么就确定是leader分区
                  //否则就是follower分区
          ...
          // 定义high watermark的checkpoint
          val highWatermarkCheckpoints = new LazyOffsetCheckpoints(this.highWatermarkCheckpoints)
          //todo:判断如果leader节点不为空，那么就成为leader节点
          val partitionsBecomeLeader = if (partitionsToBeLeader.nonEmpty)
            makeLeaders(controllerId, controllerEpoch, partitionsToBeLeader, correlationId, responseMap,
              highWatermarkCheckpoints, topicIdFromRequest)
          else
            Set.empty[Partition]
          //todo：判断如果follower节点不为空，那么就成为follower节点
          val partitionsBecomeFollower = if (partitionsToBeFollower.nonEmpty)
            //3、从这里开始同步副本数据
            //todo：如果一个节点成为follower节点，那么肯定就要去同步数据了。
   ...
   *
   *
   *  第一步：从leader partitions当中移除当前分区
   *  第二步：将分区标记为follower分区，以便客户端生产的数据再不会写入到该分区
   *   第三步：将之前这些分区的同步线程停掉，避免分区线程继续同步之前的数据
   *   第四步：将该分区的日志以及checkpoint全部都清空
   *   第五步：将生产者以及数据同步线程都停止掉
   *    第六步：如果broker正常运行，将fetcher线程添加到新的leader分区上面去
  ...
  private def makeFollowers(controllerId: Int,
    ...
    try {
      ...
      //从LeaderAndIsrRequest当中删除leader
      partitionStates.forKeyValue { (partition, partitionState) =>
      ...
      // position correctly.
      //todo：准备启动同步线程之前，必须先停止之前如果已经存在的同步线程
