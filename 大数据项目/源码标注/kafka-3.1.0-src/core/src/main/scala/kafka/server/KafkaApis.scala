...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  override def handle(request: RequestChannel.Request, requestLocal: RequestLocal): Unit = {
    try {
      ...
      request.header.apiKey match {
          //todo：使用模式匹配处理生产者发送过来的各种请求
        case ApiKeys.PRODUCE => handleProduceRequest(request, requestLocal)
          //todo：这里开始处理从节点的数据同步请求
  ...
  def handleLeaderAndIsrRequest(request: RequestChannel.Request): Unit = {
    //判断能否从zk上面获取到元数据，因为这里是运行的zk模式，所以必须要从zk上面获取到元数据，获取不到就直接抛出异常
    val zkSupport = metadataSupport.requireZkOrThrow(KafkaApis.shouldNeverReceive(request))
    // ensureTopicExists is only for client facing requests
    // We can't have the ensureTopicExists check here since the controller sends it as an advisory to all brokers so they
    // stop serving data to clients for the topic being deleted
    val correlationId = request.header.correlationId
    //将请求转换成为LeaderAndIsrRequest 这种请求类型
    val leaderAndIsrRequest = request.body[LeaderAndIsrRequest]
    //todo:权限校验，校验该请求，符合集群的操作
    authHelper.authorizeClusterOperation(request, CLUSTER_ACTION)

    /**
     * 这里进行判断，判断Broker的Epoch状态，如果broker重新启动的速度非常快，
     * 那么broker仍然可能接收到上述类型的请求
     */
    if (isBrokerEpochStale(zkSupport, leaderAndIsrRequest.brokerEpoch)) {
      ...
      //2、从这里开始同步副本数据，通过replicaManager调用becomeLeaderOrFollower来处理该请求
  ...
  def handleFetchRequest(request: RequestChannel.Request): Unit = {
    ...
    if (interesting.isEmpty)
      ...
      //在这里通过replicaManager.fetchMessages这个方法来响应fetch请求
      replicaManager.fetchMessages(
        fetchRequest.maxWait.toLong,   // 最大等待时间
        fetchRequest.replicaId,  // 副本 id，客户端为 Consumer 则为 -1
        fetchMinBytes, // 请求中的最小字节数
        fetchMaxBytes,// 请求中的最大字节数
