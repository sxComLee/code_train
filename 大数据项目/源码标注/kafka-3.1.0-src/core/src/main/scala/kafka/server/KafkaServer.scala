...
class KafkaServer(
   ...
   * 启动kafka服务端，在这里主要启动了LogManager用于日志管理，SocektServer，以及客户端请求处理等多个线程
   */
    ...
    try {
      ...
      if (canStartup) {
        ...
        * */
        ...
        /* start log manager
        * kafka的日志管理器，所有日志的清理，删除等操作都是靠它来实现的
        * */
        ...
        //调用了logManager的startup方法，入口在这里执行了加载数据的操作
        ...
        //todo:在这里构建了一个对象叫做SocketServer的线程，主要用于接收客户端的请求以及处理客户端的请求
        //其中接收客户端的请求就是Acceptor线程，处理客户端的请求就是Processor线程
        socketServer = new SocketServer(config, metrics, time, credentialProvider, apiVersionManager)
        socketServer.startup(startProcessingRequests = false)

        /* start replica manager
        * todo：kafka的ISR列表动态维护线程
        * */
        ...
        //todo：给broker的元数据进行checkPoint操作，便于恢复
        ...
        * */
        ...
        //todo：开始启动Processor以及Acceptor线程
