...
class UnifiedLog(@volatile var logStartOffset: Long,
  ...
  private def append(records: MemoryRecords,
    ...
    //如果没有合格的格式的数据，直接返回，也就是说数据格式都不合法，不让添加
    if (appendInfo.shallowCount == 0) appendInfo
    else {
      // trim any invalid bytes or partial messages before appending it to the on-disk log
      //todo:消息格式调整校验，删除无效格式消息或者无效字节，得到合法的数据。
      ...
      lock synchronized {
        maybeHandleIOException(s"Error while appending records to $topicPartition in dir ${dir.getParent}") {
          ...
          if (validateAndAssignOffsets) {
            ...
            //todo:使用当前LEO作为待写入消息集合中的第一条消息的位移值
          ...
          //todo：判断如何segment文件段已经写满了，那么就滚动一下日志文件，生成一个新的文件
          //todo: 执行日志切分，获取一个可用的segment
   ...
   * 3. The index is full
   * 产生新的segment文件段有以下几个条件，满足任意一个都行
   * 1、segment文件写满了，默认1GB
   * 2、segment文件段中的第一条消息的时间已经过时
   * 3、索引文件写入满了
   *
  ...
  private def maybeRoll(messagesSize: Int, appendInfo: LogAppendInfo): LogSegment = lock synchronized {
    ...
    } else {  //todo:他就是可用的segment
      //todo:直接返回该可用的segment
      segment
    }
  }
