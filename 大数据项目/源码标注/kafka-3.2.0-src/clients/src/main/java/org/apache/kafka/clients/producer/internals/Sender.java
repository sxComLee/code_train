...
public class Sender implements Runnable {
     ...
     */
    public Sender(LogContext logContext, KafkaClient client, ProducerMetadata metadata, RecordAccumulator accumulator,
                  boolean guaranteeMessageOrder, int maxRequestSize, short acks, int retries, SenderMetricsRegistry metricsRegistry,
                  Time time, int requestTimeoutMs, long retryBackoffMs, TransactionManager transactionManager,
    ...
    private List<ProducerBatch> getExpiredInflightBatches(long now) {
        ...
        for (Iterator<Map.Entry<TopicPartition, List<ProducerBatch>>> batchIt = inFlightBatches.entrySet()
                .iterator(); batchIt.hasNext(); ) {
                    ...
                    if (batch.hasReachedDeliveryTimeout(accumulator.getDeliveryTimeoutMs(), now)) {
                        ...
                        if (!batch.isDone()) {
                            ...
                            throw new IllegalStateException(
                                    batch.topicPartition + " batch created at " + batch.createdMs + " gets unexpected final state " + batch.finalState());
                        }
                    } else {
    ...
    void runOnce() {
        ...
        if (transactionManager != null) {
            try {
                ...
                /***********************************************************************
                 * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
                 *  注释： 如果 transactionManager 工作不正常了。
                 */
                if (transactionManager.hasFatalError()) {
                    RuntimeException lastError = transactionManager.lastError();
                    if (lastError != null) maybeAbortBatches(lastError);
                ...
                /***********************************************************************
                 * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
                 *  注释： 检查我们是否需要一个新的 producerId。如果是这样，我们将排队一个 InitProducerId 请求，该请求将在下面发送
                 */
         ...
         * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
         *  注释： 发送生产数据
         *  1、原来将每一条要发送的数据，追加写入到 RecordAccumulator（每个 批次按照 partition 进行组织） 中，形成了额一个个的批次
         *  2、现在这个方法的目的，就是去 RecordAccumulator 去寻找有没有可以执行发送的数据
         *  如果有：则按照 broker 进行分类组织
         */
         ...
         * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
         *  注释： 处理 NIO 事件
         *  内部实现：Selector.select();  轮询是否有准备就绪的 OP_READ 和 OP_WRITE 事件
         *  1、OP_READ:  之前发送了 PRODUCE 这种类别的请求给 Borker，并且 Broker 完成了处理返回了响应，客户端处理 OP_READ 事件，读取到响应数据
         *  2、OP_WRITE： 上述的 sendProducerData(currentTimeMs); 执行过程中，会注册 OP_WRITE 事件表示待发送的批次数据已经准备就绪，可以通过网络执行发送了。
         */
     ...
     * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
     *  注释： 内部分三步走：
     *  1、获取可以执行发送的 ProducerBatch
     *  2、添加到 InflightBatches 集合中，表示正在处理
     *  3、完成这些 ProducerBatch 的发送，真正发送请求，将数据发送到 Broker
     */
    private long sendProducerData(long now) {
        ...
        /***********************************************************************
         * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
         *  注释： 第一步： 获取可以执行发送的 ProducerBatch
         */
        Map<Integer, List<ProducerBatch>> batches = this.accumulator.drain(cluster, result.readyNodes, this.maxRequestSize, now);

        /***********************************************************************
         * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
         *  注释： 第二步： 添加到 InflightBatches 集合中，表示正在处理
         */
        ...
        if (!expiredBatches.isEmpty()) log.trace("Expired {} batches in accumulator", expiredBatches.size());
        ...
        /***********************************************************************
         * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
         *  注释： 第三步： 完成这些 ProducerBatch 的发送，真正发送请求，将数据发送到 Broker
         */
    ...
    private boolean maybeSendAndPollTransactionalRequest() {
        ...
        if (nextRequestHandler == null) return false;

        AbstractRequest.Builder<?> requestBuilder = nextRequestHandler.requestBuilder();
        Node targetNode = null;
        try {
            FindCoordinatorRequest.CoordinatorType coordinatorType = nextRequestHandler.coordinatorType();
            targetNode = coordinatorType != null ? transactionManager.coordinator(coordinatorType) : client.leastLoadedNode(
                    time.milliseconds());
            if (targetNode != null) {
                ...
                log.trace("Coordinator not known for {}, will retry {} after finding coordinator.", coordinatorType,
            ...
            log.debug("Sending transactional request {} to node {} with correlation ID {}", requestBuilder, targetNode,
            ...
            log.debug("Disconnect from {} while trying to send request {}. Going " + "to back off and retry.", targetNode,
                    requestBuilder, e);
        ...
        if (response.wasDisconnected()) {
            ...
            for (ProducerBatch batch : batches.values())
                completeBatch(batch, new ProduceResponse.PartitionResponse(Errors.NETWORK_EXCEPTION,
                        String.format("Disconnected from node %s", response.destination())), correlationId, now);
        } else if (response.versionMismatch() != null) {
            log.warn("Cancelled request {} due to a version mismatch with node {}", response, response.destination(),
                        ...
                        .forEach(r -> r.partitionResponses()
                                .forEach(p -> {
                                    ...
                                    ProduceResponse.PartitionResponse partResp = new ProduceResponse.PartitionResponse(
                                            Errors.forCode(p.errorCode()), p.baseOffset(), p.logAppendTimeMs(), p.logStartOffset(),
                                            p.recordErrors()
                                                    .stream()
                                                    .map(e -> new ProduceResponse.RecordError(e.batchIndex(),
                                                            e.batchIndexErrorMessage()))
                                                    .collect(Collectors.toList()), p.errorMessage());
    ...
    private void completeBatch(ProducerBatch batch, ProduceResponse.PartitionResponse response, long correlationId, long now) {
        ...
        if (error == Errors.MESSAGE_TOO_LARGE && batch.recordCount > 1 && !batch.isDone() && (batch.magic() >= RecordBatch.MAGIC_VALUE_V2 || batch.isCompressed())) {
            ...
            log.warn(
                    ...
                    correlationId, batch.topicPartition, this.retries - batch.attempts(), formatErrMsg(response));
            if (transactionManager != null) transactionManager.removeInFlightBatch(batch);
            ...
            if (canRetry(batch, response, now)) {
                log.warn(
                        ...
                        correlationId, batch.topicPartition, this.retries - batch.attempts() - 1, formatErrMsg(response));
            ...
            if (error.exception() instanceof InvalidMetadataException) {
                if (error.exception() instanceof UnknownTopicOrPartitionException) {
                    ...
                    log.warn(
                            ...
                            batch.topicPartition, error.exception(response.errorMessage)
                                    .toString());
                }
                metadata.requestUpdate();
            }
    ...
    private String formatErrMsg(ProduceResponse.PartitionResponse response) {
        String errorMessageSuffix = (response.errorMessage == null || response.errorMessage.isEmpty()) ? "" : String.format(
                ". Error Message: %s", response.errorMessage);
    ...
    private void failBatch(ProducerBatch batch, ProduceResponse.PartitionResponse response, boolean adjustSequenceNumbers) {
        ...
        else topLevelException = response.error.exception(response.errorMessage);
    ...
    private void failBatch(ProducerBatch batch, RuntimeException topLevelException, boolean adjustSequenceNumbers) {
        failBatch(batch, topLevelException, batchIndex -> topLevelException, adjustSequenceNumbers);
    }

    private void failBatch(ProducerBatch batch, RuntimeException topLevelException,
                           Function<Integer, RuntimeException> recordExceptions, boolean adjustSequenceNumbers) {
        if (transactionManager != null) {
    ...
    private boolean canRetry(ProducerBatch batch, ProduceResponse.PartitionResponse response, long now) {
        return !batch.hasReachedDeliveryTimeout(accumulator.getDeliveryTimeoutMs(),
                now) && batch.attempts() < this.retries && !batch.isDone() && (transactionManager == null ? response.error.exception() instanceof RetriableException : transactionManager.canRetry(
                response, batch)
        );
    }
    ...
    private void sendProduceRequests(Map<Integer, List<ProducerBatch>> collated, long now) {
        ...
        // TODO_MA 马中华 注释： 每遍历一次，表示往一个 Borker 发送所有的 批次
    ...
    private void sendProduceRequest(long now, int destination, short acks, int timeout, List<ProducerBatch> batches) {
        ...
        // TODO_MA 马中华 注释： 构建一个数据容器
        ProduceRequestData.TopicProduceDataCollection tpd = new ProduceRequestData.TopicProduceDataCollection();

        /***********************************************************************
         * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
         *  注释： 遍历要发送的每一批次数据 ProducerBatch
         */
        for (ProducerBatch batch : batches) {
            TopicPartition tp = batch.topicPartition;

            // TODO_MA 马中华 注释： 获取每个 ProducerBatch 中的所有的 Record
            ...
            if (!records.hasMatchingMagic(minUsedMagic)) records = batch.records()
                    .downConvert(minUsedMagic, 0, time)
                    .records();

            // TODO_MA 马中华 注释： 构建一个 ProduceRequestData.TopicProduceData，用来盛放 Records
            ...
            // TODO_MA 马中华 注释： 将数据 Records 加入到  ProduceRequestData.TopicProduceData 中
            tpData.partitionData()
                    .add(new ProduceRequestData.PartitionProduceData().setIndex(tp.partition())
                            .setRecords(records));

            // TODO_MA 马中华 注释： 根据分区进行管理
            recordsByPartition.put(tp, batch);
        }

        // TODO_MA 马中华 注释：  数据预处理搞定了
        ...
        // TODO_MA 马中华 注释： 将  ProduceRequestData.TopicProduceData 加入到 ProduceRequest.Builder
        // TODO_MA 马中华 注释： 将来会放在 ClientRequest 中传递给下游去构造 InFlightRequest
        // TODO_MA 马中华 注释： 注意： 这里会将请求标识为 ApiKeys.PRODUCE，意思是生产数据
        ProduceRequest.Builder requestBuilder = ProduceRequest.forMagic(minUsedMagic, new ProduceRequestData().setAcks(acks)
         ...
         * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
         *  注释： 发送请求
         *  其实这个地方，只是准备好了请求数据，然后注册了 OP_WRITE 事件
         *  等待 selector.select() 轮询到该 OP_WRITE 事件，然后再集中完成事件处理
         */
        client.send(clientRequest, now);
        log.trace("Sent produce request to {}: {}", nodeId, requestBuilder);

        // TODO_MA 马中华 注释： 到此为止，如果不考虑 PRC 具体发送数据的机制，那么 这个 PRODUCE 的请求发送给了 Broker
    }
    ...
    private static class SenderMetrics {
        ...
        public SenderMetrics(SenderMetricsRegistry metrics, Metadata metadata, KafkaClient client, Time time) {
            ...
            this.metrics.addMetric(metrics.metadataAge, (config, now) -> (now - metadata.lastSuccessfulUpdate()) / 1000.0);
        ...
        public void recordRetries(String topic, int count) {
            ...
            if (topicRetrySensor != null) topicRetrySensor.record(count, now);
        }
        ...
        public void recordErrors(String topic, int count) {
            ...
            if (topicErrorSensor != null) topicErrorSensor.record(count, now);
        }
        ...
        public void recordLatency(String node, long latency) {
            ...
            if (!node.isEmpty()) {
                ...
                if (nodeRequestTime != null) nodeRequestTime.record(latency, now);
            }
        }
