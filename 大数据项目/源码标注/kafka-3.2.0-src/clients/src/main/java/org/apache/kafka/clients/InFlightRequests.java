...
final class InFlightRequests {
    ...
    // TODO_MA 马中华 注释： 每个 Connection 链接上能缓存的 InFlightRequest 的个数
     ...
     *  注释： 所有的请求
     *  String = Broker ID
     *  Deque<NetworkClient.InFlightRequest> 存储的是待发送给某一个 Broker 的请求队列
     */
    ...
    public void add(NetworkClient.InFlightRequest request) {
        ...
        // TODO_MA 马中华 注释： Broker 信息
        String destination = request.destination;

        // TODO_MA 马中华 注释： 该 Broker 对应的队列
        Deque<NetworkClient.InFlightRequest> reqs = this.requests.get(destination);

        // TODO_MA 马中华 注释： 如果不存在，则创建一个
        if (reqs == null) {
            reqs = new ArrayDeque<>();
            this.requests.put(destination, reqs);
        }

        // TODO_MA 马中华 注释： 请求加入队列
