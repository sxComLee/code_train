...
public class ConsumerNetworkClient implements Closeable {
     ...
     *  注释：
     */
    public ConsumerNetworkClient(LogContext logContext, KafkaClient client, Metadata metadata, Time time, long retryBackoffMs,
                                 int requestTimeoutMs, int maxPollTimeoutMs) {
        ...
        // TODO_MA 马中华 注释： poll 数据的超时时间： 5000, 可以通过 heartbeat.interval.ms 配置
        this.maxPollTimeoutMs = Math.min(maxPollTimeoutMs, MAX_POLL_TIMEOUT_MS);

        // TODO_MA 马中华 注释： 请求超时时间： request.timeout.ms
    ...
    public RequestFuture<ClientResponse> send(Node node, AbstractRequest.Builder<?> requestBuilder, int requestTimeoutMs) {
        ...
        // TODO_MA 马中华 注释： 唤醒线程开始完成 Request 对象发送给 KafkaServer
        client.wakeup();
    ...
    public void poll(Timer timer, PollCondition pollCondition, boolean disableWakeup) {
        ...
        try {
            ...
            if (pendingCompletion.isEmpty() && (pollCondition == null || pollCondition.shouldBlock())) {
                ...
                if (client.inFlightRequestCount() == 0) pollTimeout = Math.min(pollTimeout, retryBackoffMs);
    ...
    private void firePendingCompletedRequests() {
        ...
        for (; ; ) {
            ...
            if (completionHandler == null) break;
        ...
        for (Node node : unsent.nodes()) {
            if (client.connectionFailed(node)) {
                ...
                for (ClientRequest request : requests) {
                    ...
                    handler.onComplete(new ClientResponse(request.makeHeader(request.requestBuilder()
                            .latestAllowedVersion()), request.callback(), request.destination(), request.createdTimeMs(), now, true,
    ...
    long trySend(long now) {
        ...
        for (Node node : unsent.nodes()) {
            ...
            if (iterator.hasNext()) pollDelayMs = Math.min(pollDelayMs, client.pollDelayMs(node, now));
    ...
    private class RequestFutureCompletionHandler implements RequestCompletionHandler {
        ...
        public void fireCompletion() {
            if (e != null) {
                ...
                log.debug("Cancelled request with header {} due to node {} being disconnected", response.requestHeader(),
    ...
    private static final class UnsentRequests {
        ...
        public void clean() {
            ...
            synchronized (unsent) {
                Iterator<ConcurrentLinkedQueue<ClientRequest>> iterator = unsent.values()
                        .iterator();
                while (iterator.hasNext()) {
