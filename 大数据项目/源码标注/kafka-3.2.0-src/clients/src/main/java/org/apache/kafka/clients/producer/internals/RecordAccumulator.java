...
public class RecordAccumulator {
    ...
    /***********************************************************************
     * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
     *  注释： 一个 ProducerBatch 就是 Producer 一次向 Broker 发送的数据包，包含了多条 Message
     *  一个 Producer 可能会往多个 Broker 的多个 分区发送数据，所以所有待发送的数据就按照 分区进行组织
     *  然后进行批次有序发送
     *  1、分区定义
     *  2、发送给该分区的 批次数据 队列
     */
     ...
     */
    public RecordAccumulator(LogContext logContext, int batchSize, CompressionType compression, int lingerMs, long retryBackoffMs,
                             int deliveryTimeoutMs, Metrics metrics, String metricGrpName, Time time, ApiVersions apiVersions,
                             TransactionManager transactionManager, BufferPool bufferPool) {
    ...
    public RecordAppendResult append(TopicPartition tp, long timestamp, byte[] key, byte[] value, Header[] headers,
                                     Callback callback, long maxTimeToBlock, boolean abortOnNewBatch,
        ...
        try {
            ...
            /***********************************************************************
             * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
             *  注释： 获取该 TopicPartition 对应的 Dequeue
             */
            ...
            log.trace("Allocating a new {} byte message buffer for topic {} partition {} with remaining timeout {}ms", size,
                    tp.topic(), tp.partition(), maxTimeToBlock);

            /***********************************************************************
             * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
             *  注释： 如果上面的追加写没成功，则代码会走到这儿，为什么没成功了，大概率是内存不够
             *  1、BufferPool（内存池/ByteBufer队列 +  未池化内存） free 申请一个 ByteBuffer 的时候，不一定是 16kb
             *      如果刚才要写的数据 大于 16KB
             *  2、申请内存
             *      1、如果待写入的数据小于等于 16kb 的话，从 内存池中申请看是否有可用的，如果有，拿来直接用
             *      如果么有的话，则从未池化内存区域中去申请
             *      2、如果待写入的数据大于 16kb 的话 ， 直接从未池化内存去申请
             *          万一未池化内存区域总大小不够了，驱使内存池中的ByteBuffer回收
             *  3、BufferPool 默认大小： 32M，同构 buffer.memory 去更改
             */
            ...
            synchronized (dq) {
                ...
                /***********************************************************************
                 * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
                 *  注释： 利用刚才申请到的 ByteBuffer 创建一个新的 ProducerBatch
                 *  1、buffer 实际的一块内存区域，用来存储真实的数据
                 *  2、MemoryRecordsBuilder
                 *  3、MemoryRecords：表示这个 ProducerBatch 中的所有数据
                 *  4、ProducerBatch 在逻辑上是由一堆 Record 组成的，ProducerBatch 不仅仅只是包含 MemoryRecords
                 */
                ...
                /***********************************************************************
                 * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
                 *  注释： 将 ProducerRecord 写进去 ProducerBatch 中
                 */
                ...
                /***********************************************************************
                 * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
                 *  注释： 将 ProducerBatch 丢进去 ProducerBatch 队列的末尾
                 */
                dq.addLast(batch);
     ...
     */
    private RecordAppendResult tryAppend(long timestamp, byte[] key, byte[] value, Header[] headers, Callback callback,
                                         Deque<ProducerBatch> deque, long nowMs) {

        // TODO_MA 马中华 注释： 从 Dequeue 中拿到最后一个 ProducerBatch
        ProducerBatch last = deque.peekLast();
        if (last != null) {

            /***********************************************************************
             * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
             *  注释： 将 Key value 追加写到 ProducerBatch 中
             */
            ...
            else return new RecordAppendResult(future, deque.size() > 1 || last.isFull(), false, false);
        }
        return null;
    }
    ...
    private void insertInSequenceOrder(Deque<ProducerBatch> deque, ProducerBatch batch) {
        ...
        if (batch.baseSequence() == RecordBatch.NO_SEQUENCE) throw new IllegalStateException(
                "Trying to re-enqueue a batch which doesn't have a sequence even " + "though idempotency is enabled.");

        if (transactionManager.nextBatchBySequence(batch.topicPartition) == null) throw new IllegalStateException(
                "We are re-enqueueing a batch which is not tracked as part of the in flight " + "requests. batch.topicPartition: " + batch.topicPartition + "; batch.baseSequence: " + batch.baseSequence());
        ...
        if (firstBatchInQueue != null && firstBatchInQueue.hasSequence() && firstBatchInQueue.baseSequence() < batch.baseSequence()) {
            ...
            while (deque.peekFirst() != null && deque.peekFirst()
                    .hasSequence() && deque.peekFirst()
                    .baseSequence() < batch.baseSequence()) orderedBatches.add(deque.pollFirst());

            log.debug("Reordered incoming batch with sequence {} for partition {}. It was placed in the queue at " + "position {}",
                    batch.baseSequence(), batch.topicPartition, orderedBatches.size());
    ...
    /***********************************************************************
     * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
     *  注释： 假设 目标 topic 有 50 个分区
     *  RecordAccumulator 中的 map 的 key-value 的个数 = 50 个
     */
    private List<ProducerBatch> drainBatchesForOneNode(Cluster cluster, Node node, int maxSize, long now) {
        ...
        // TODO_MA 马中华 注释： 依次遍历每个 partition
        // TODO_MA 马中华 注释： 其实就是map的每个key
        ...
        do {
            ...
            // TODO_MA 马中华 注释： 获取队列， 就是获取map的value
            ...
            synchronized (deque) {
                ...
                if (size + first.estimatedSizeInBytes() > maxSize && !ready.isEmpty()) {
                    ...
                    if (shouldStopDrainBatchesForPartition(first, tp)) break;
                }

                // TODO_MA 马中华 注释： 获取队列首的 ProducerBatch
                ...
                if (producerIdAndEpoch != null && !batch.hasSequence()) {
                    ...
                    log.debug(
                            ...
                            producerIdAndEpoch.producerId, producerIdAndEpoch.epoch, batch.baseSequence(), tp);
            ...
            // TODO_MA 马中华 注释： ProducerBatch 构建完成，生成 MemoryRecords
            // TODO_MA 马中华 注释： ProducerBatch 内部有一个 MemoryRecordsBuilder 用来完成 key value 到 ByteBuffer 中的写入
            // TODO_MA 马中华 注释： 内部真正写的时候，是通过一个输出流
            // TODO_MA 马中华 注释： 把这个 ProducerBatch 形成为一个完整的能执行发送的批次
            // TODO_MA 马中华 注释： 1、flush
            // TODO_MA 马中华 注释： 2、写头信息
            batch.close();
    ...
    public Map<Integer, List<ProducerBatch>> drain(Cluster cluster, Set<Node> nodes, int maxSize, long now) {
        ...
        /***********************************************************************
         * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
         *  注释： 从 RecordAccumulator 中获取 ProducerBatch
         */
        Map<Integer, List<ProducerBatch>> batches = new HashMap<>();

        // TODO_MA 马中华 注释： nodes = ready nodes， 能正常通信，符合发送数据要求的
        for (Node node : nodes) {

            // TODO_MA 马中华 注释： 遍历每个 Borker，找出来要发送给这个 Borker 的所有的批次
            List<ProducerBatch> ready = drainBatchesForOneNode(cluster, node, maxSize, now);

            // TODO_MA 马中华 注释： 登记到这个map中
    ...
    private Deque<ProducerBatch> getOrCreateDeque(TopicPartition tp) {
        ...
        // TODO_MA 马中华 注释： 根据分区定义到 batches 中去找
        Deque<ProducerBatch> d = this.batches.get(tp);

        // TODO_MA 马中华 注释： 找到了，直接返回
        if (d != null) return d;

        // TODO_MA 马中华 注释： 否则创建一个新的
        d = new ArrayDeque<>();

        // TODO_MA 马中华 注释： 加入到 batches 中
        Deque<ProducerBatch> previous = this.batches.putIfAbsent(tp, d);

        // TODO_MA 马中华 注释： 返回 batches 中拿到的 Deque<ProducerBatch>
