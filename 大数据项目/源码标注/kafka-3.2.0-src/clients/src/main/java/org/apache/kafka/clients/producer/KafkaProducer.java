...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    public KafkaProducer(Map<String, Object> configs, Serializer<K> keySerializer, Serializer<V> valueSerializer) {
         ...
         *  注释：
         */
        this(new ProducerConfig(ProducerConfig.appendSerializerToConfig(configs, keySerializer, valueSerializer)), keySerializer,
                valueSerializer, null, null, null, Time.SYSTEM);
    }
    ...
    KafkaProducer(ProducerConfig config, Serializer<K> keySerializer, Serializer<V> valueSerializer, ProducerMetadata metadata,
                  KafkaClient kafkaClient, ProducerInterceptors<K, V> interceptors, Time time) {
        try {
            this.producerConfig = config;
            this.time = time;

            // TODO_MA 马中华 注释： transactional.id
            String transactionalId = config.getString(ProducerConfig.TRANSACTIONAL_ID_CONFIG);

            // TODO_MA 马中华 注释： client.id
            this.clientId = config.getString(ProducerConfig.CLIENT_ID_CONFIG);

            LogContext logContext;
            if (transactionalId == null) logContext = new LogContext(String.format("[Producer clientId=%s] ", clientId));
            else logContext = new LogContext(
            ...
            List<MetricsReporter> reporters = config.getConfiguredInstances(ProducerConfig.METRIC_REPORTER_CLASSES_CONFIG,
                    MetricsReporter.class, Collections.singletonMap(ProducerConfig.CLIENT_ID_CONFIG, clientId));
            ...
            /***********************************************************************
             * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
             *  注释： partitioner.class 程序根据此参数设置 数据分区器
             */
            ...
            if (keySerializer == null) {
                ...
                this.keySerializer.configure(config.originals(Collections.singletonMap(ProducerConfig.CLIENT_ID_CONFIG, clientId)),
                        true);
            } else {
            ...
            /***********************************************************************
             * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
             *  注释： KafkaProducer 生产端的拦截器设置
             */
            List<ProducerInterceptor<K, V>> interceptorList = (List) config.getConfiguredInstances(
                    ProducerConfig.INTERCEPTOR_CLASSES_CONFIG, ProducerInterceptor.class,
                    Collections.singletonMap(ProducerConfig.CLIENT_ID_CONFIG, clientId));
            if (interceptors != null) this.interceptors = interceptors;
            else this.interceptors = new ProducerInterceptors<>(interceptorList);

            ClusterResourceListeners clusterResourceListeners = configureClusterResourceListeners(keySerializer, valueSerializer,
                    interceptorList, reporters);

            // TODO_MA 马中华 注释： max.request.size
            this.maxRequestSize = config.getInt(ProducerConfig.MAX_REQUEST_SIZE_CONFIG);
            // TODO_MA 马中华 注释： buffer.memory
            this.totalMemorySize = config.getLong(ProducerConfig.BUFFER_MEMORY_CONFIG);
            // TODO_MA 马中华 注释： compression.type
            this.compressionType = CompressionType.forName(config.getString(ProducerConfig.COMPRESSION_TYPE_CONFIG));
            // TODO_MA 马中华 注释： max.block.ms
            ...
            /***********************************************************************
             * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
             *  注释： 初始化一个用来组织 ProducerRecord 进行分区分批次发送的内存缓冲
             */
            this.accumulator = new RecordAccumulator(logContext,
                    // TODO_MA 马中华 注释： batch.size
                    config.getInt(ProducerConfig.BATCH_SIZE_CONFIG), this.compressionType,
                    // TODO_MA 马中华 注释： linger.ms
                    lingerMs(config), retryBackoffMs, deliveryTimeoutMs, metrics, PRODUCER_METRIC_GROUP_NAME, time, apiVersions,
                    transactionManager,
                    // TODO_MA 马中华 注释： 内存池
                    new BufferPool(this.totalMemorySize, config.getInt(ProducerConfig.BATCH_SIZE_CONFIG), metrics, time,
                            PRODUCER_METRIC_GROUP_NAME));

            // TODO_MA 马中华 注释： bootstrap.servers，client.dns.lookup
            ...
            /***********************************************************************
             * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
             *  注释： ProducerMetadata 是存在于客户端用来去管理和维护 元数据的 工作组件
             */
            if (metadata != null) {
                this.metadata = metadata;
            } else {
                this.metadata = new ProducerMetadata(retryBackoffMs,
                        // TODO_MA 马中华 注释： metadata.max.age.ms
                        config.getLong(ProducerConfig.METADATA_MAX_AGE_CONFIG),
                        // TODO_MA 马中华 注释： metadata.max.idle.ms
                        config.getLong(ProducerConfig.METADATA_MAX_IDLE_CONFIG), logContext, clusterResourceListeners, Time.SYSTEM);
            ...
            /***********************************************************************
             * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
             *  注释： 创建 Sender 线程用于完成 Message 按批次有序发送到 Broker
             */
    ...
    // visible for testing
    KafkaProducer(ProducerConfig config, LogContext logContext, Metrics metrics, Serializer<K> keySerializer,
                  Serializer<V> valueSerializer, ProducerMetadata metadata, RecordAccumulator accumulator,
                  TransactionManager transactionManager, Sender sender, ProducerInterceptors<K, V> interceptors,
                  Partitioner partitioner, Time time, KafkaThread ioThread) {
        this.producerConfig = config;
        this.time = time;
        this.clientId = config.getString(ProducerConfig.CLIENT_ID_CONFIG);
    ...
    Sender newSender(LogContext logContext, KafkaClient kafkaClient, ProducerMetadata metadata) {
        ...
        // TODO_MA 马中华 注释： max.in.flight.requests.per.connection, 默认是：5
        int maxInflightRequests = producerConfig.getInt(ProducerConfig.MAX_IN_FLIGHT_REQUESTS_PER_CONNECTION);

        // TODO_MA 马中华 注释： request.timeout.ms
        ...
        /***********************************************************************
         * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
         *  注释： 初始化一个用来和 Broker 完成网路链接并且完成真实数据发送的网络客户端
         */
        KafkaClient client = kafkaClient != null ? kafkaClient : new NetworkClient(
                // TODO_MA 马中华 注释： connections.max.idle.ms
                new Selector(producerConfig.getLong(ProducerConfig.CONNECTIONS_MAX_IDLE_MS_CONFIG), this.metrics, time, "producer",
                        channelBuilder, logContext), metadata, clientId, maxInflightRequests,
                // TODO_MA 马中华 注释： reconnect.backoff.ms
                producerConfig.getLong(ProducerConfig.RECONNECT_BACKOFF_MS_CONFIG),
                // TODO_MA 马中华 注释： reconnect.backoff.max.ms
                producerConfig.getLong(ProducerConfig.RECONNECT_BACKOFF_MAX_MS_CONFIG),
                // TODO_MA 马中华 注释： send.buffer.bytes
                producerConfig.getInt(ProducerConfig.SEND_BUFFER_CONFIG),
                // TODO_MA 马中华 注释： receive.buffer.bytes
                producerConfig.getInt(ProducerConfig.RECEIVE_BUFFER_CONFIG), requestTimeoutMs,
                // TODO_MA 马中华 注释： socket.connection.setup.timeout.ms
                producerConfig.getLong(ProducerConfig.SOCKET_CONNECTION_SETUP_TIMEOUT_MS_CONFIG),
                // TODO_MA 马中华 注释： socket.connection.setup.timeout.max.ms
                producerConfig.getLong(ProducerConfig.SOCKET_CONNECTION_SETUP_TIMEOUT_MAX_MS_CONFIG), time, true, apiVersions,
        ...
        /***********************************************************************
         * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
         *  注释： 构造一个 Sender 线程专门用来获取 RecordAccumulator 中的 ProducerBatch 来完成发送的工作组件
         */
        return new Sender(logContext, client, metadata, this.accumulator, maxInflightRequests == 1,
                ...
                // TODO_MA 马中华 注释： retries
                producerConfig.getInt(ProducerConfig.RETRIES_CONFIG), metricsRegistry.senderMetrics, time, requestTimeoutMs,
                // TODO_MA 马中华 注释： retry.backoff.ms
                producerConfig.getLong(ProducerConfig.RETRY_BACKOFF_MS_CONFIG), this.transactionManager, apiVersions);
    }
    ...
    private static int configureDeliveryTimeout(ProducerConfig config, Logger log) {
        ...
        // TODO_MA 马中华 注释： delivery.timeout.ms
        ...
        if (deliveryTimeoutMs < lingerAndRequestTimeoutMs) {
            if (config.originals()
                    .containsKey(ProducerConfig.DELIVERY_TIMEOUT_MS_CONFIG)) {
                // throw an exception if the user explicitly set an inconsistent value
                throw new ConfigException(
                        ProducerConfig.DELIVERY_TIMEOUT_MS_CONFIG + " should be equal to or larger than " + ProducerConfig.LINGER_MS_CONFIG + " + " + ProducerConfig.REQUEST_TIMEOUT_MS_CONFIG);
            } else {
    ...
    private TransactionManager configureTransactionState(ProducerConfig config, LogContext logContext) {
        ...
        if (config.getBoolean(ProducerConfig.ENABLE_IDEMPOTENCE_CONFIG)) {
            ...
            if (transactionManager.isTransactional()) log.info("Instantiated a transactional producer.");
            else log.info("Instantiated an idempotent producer.");
        }
        ...
        try {
            ...
            try {
                ...
                throw new SerializationException("Can't convert key of class " + record.key()
                        .getClass()
                        .getName() + " to class " + producerConfig.getClass(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG)
                        .getName() + " specified in key.serializer", cce);
            }
            ...
            try {
                ...
                throw new SerializationException("Can't convert value of class " + record.value()
                        .getClass()
                        .getName() + " to class " + producerConfig.getClass(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG)
                        .getName() + " specified in value.serializer", cce);
            }

            /***********************************************************************
             * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
             *  注释： 计算获取到 该条要发送的数据 对应的 Topic 的分区的编号
             */
            ...
            /***********************************************************************
             * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
             *  注释： 先估算一下这条要发送的数据的大小上限。
             *  如果这条数据太大，大于 16KB 的话，需要从 未池化内存中，单独申请一块内存来容纳
             *  一条数据的最小大小是：61 + 21 = 82
             *  如果要发送的数据大小是 100byte，则总大小是：61 + 21 + 100 = 182byte
             */
            int serializedSize = AbstractRecords.estimateSizeInBytesUpperBound(apiVersions.maxUsableProduceMagic(), compressionType,
                    serializedKey, serializedValue, headers);
            // TODO_MA 马中华 注释： 校验，默认 record 的最大大小不能超过 1M
            ...
            /***********************************************************************
             * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
             *  注释： 将数据追加写入 RecordAccumulator
             */
            RecordAccumulator.RecordAppendResult result = accumulator.append(tp, timestamp, serializedKey, serializedValue, headers,
                    interceptCallback, remainingWaitMs, true, nowMs);

            /***********************************************************************
             * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
             *  注释： 如果由于一个 ProducerBatch 内存不够用了，则上面的 append方法会返回，然后再走下面的代码尝试另外一次 append
             */
            ...
            // TODO_MA 马中华 注释： 注意： 如果一个 ProducerBatch 写满了
            // TODO_MA 马中华 注释： 注意： 如果由于此次写 key value 导致形成了一个新的 ProducerBatch
            // TODO_MA 马中华 注释： 这都表示有一个能发送的 ProducerBatch 形成了
            // TODO_MA 马中华 注释： 所以唤醒 Sender 线程执行发送吧
            if (result.batchIsFull || result.newBatchCreated) {
                log.trace("Waking up the sender since topic {} partition {} is either full or getting a new batch", record.topic(),
                        partition);
                this.sender.wakeup();
            }
     ...
     */
    private ClusterAndWaitTime waitOnMetadata(String topic, Integer partition, long nowMs,
                                              long maxWaitMs) throws InterruptedException {
        // add topic to metadata topic list if it is not there already and reset expiry
        // TODO_MA 马中华 注释： 尝试从缓存中获取 Cluster 也就是集群元数据
        Cluster cluster = metadata.fetch();

        // TODO_MA 马中华 注释： 如果要发送数据的 Topic 是非法 topic
        if (cluster.invalidTopics()
                .contains(topic)) throw new InvalidTopicException(topic);

        metadata.add(topic, nowMs);

        // TODO_MA 马中华 注释： 计算 topic 的分区个数
        ...
        do {
            ...
            // TODO_MA 马中华 注释： 元数据拿到了之后会丢倒 metadata 中的 cache 里面
            // TODO_MA 马中华 注释： 直接从里面拿就可以了。
            cluster = metadata.fetch();
            elapsed = time.milliseconds() - nowMs;
            if (elapsed >= maxWaitMs) {
                throw new TimeoutException(
                        partitionsCount == null ? String.format("Topic %s not present in metadata after %d ms.", topic,
                                maxWaitMs) : String.format(
            ...
            // TODO_MA 马中华 注释： 最后再统计一下 topic 的 partition 的数量
    ...
    private void ensureValidRecordSize(int size) {
        if (size > maxRequestSize) throw new RecordTooLargeException(
                "The message is " + size + " bytes when serialized which is larger than " + maxRequestSize + ", which is the value of the " + ProducerConfig.MAX_REQUEST_SIZE_CONFIG + " configuration.");
        if (size > totalMemorySize) throw new RecordTooLargeException(
    ...
    private void close(Duration timeout, boolean swallowException) {
        ...
        if (timeoutMs < 0) throw new IllegalArgumentException("The timeout cannot be negative.");
        ...
        if (timeoutMs > 0) {
            if (invokedFromCallback) {
                log.warn(
                        "Overriding close timeout {} ms to 0 ms in order to prevent useless blocking due to self-join. " + "This means you have incorrectly invoked close with a non-zero timeout from the producer call-back.",
                        timeoutMs);
            } else {
                // Try to close gracefully.
                if (this.sender != null) this.sender.initiateClose();
        ...
        if (this.sender != null && this.ioThread != null && this.ioThread.isAlive()) {
            log.info(
                    "Proceeding to force close the producer since pending requests could not be completed " + "within timeout {} ms.",
                    timeoutMs);
    ...
    private ClusterResourceListeners configureClusterResourceListeners(Serializer<K> keySerializer, Serializer<V> valueSerializer,
                                                                       List<?>... candidateLists) {
    ...
    private int partition(ProducerRecord<K, V> record, byte[] serializedKey, byte[] serializedValue, Cluster cluster) {
        ...
        // TODO_MA 马中华 注释： 如果指定了 Partition，则直接用这个 Partition 信息
        Integer partition = record.partition();
        if (partition != null) {
            return partition;
        }

        // TODO_MA 马中华 注释： 否则，调用 Partitioner 去计算
        int customPartition = partitioner.partition(record.topic(), record.key(), serializedKey, record.value(), serializedValue,
                cluster);

        // TODO_MA 马中华 注释： 如果自定义的 Partitoiner 组件有问题，可能计算得到的值小于 0，那就是异常情况
    ...
    private void throwIfNoTransactionManager() {
        if (transactionManager == null) throw new IllegalStateException(
                "Cannot use transactional methods without enabling transactions " + "by setting the " + ProducerConfig.TRANSACTIONAL_ID_CONFIG + " configuration property");
    }
