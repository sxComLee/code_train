...
public class DefaultRecord implements Record {
    ...
    DefaultRecord(int sizeInBytes, byte attributes, long offset, long timestamp, int sequence, ByteBuffer key, ByteBuffer value,
                  Header[] headers) {
     ...
     * // TODO_MA 马中华 注释： 写具体的消息，具体消息格式，可以看下面这个方法的源码实现
     */
    public static int writeTo(DataOutputStream out, int offsetDelta, long timestampDelta, ByteBuffer key, ByteBuffer value,
                              Header[] headers) throws IOException {

        // TODO_MA 马中华 注释： key ,value + header 消息总长度
        int sizeInBytes = sizeOfBodyInBytes(offsetDelta, timestampDelta, key, value, headers);
        ByteUtils.writeVarint(sizeInBytes, out);

        // TODO_MA 马中华 注释： attributes  1byte
        byte attributes = 0; // there are no used record attributes at the moment
        out.write(attributes);

        // TODO_MA 马中华 注释： timestampDelta long 8byte
        ByteUtils.writeVarlong(timestampDelta, out);

        // TODO_MA 马中华 注释： offsetDelta int 4byte
        ByteUtils.writeVarint(offsetDelta, out);

        // TODO_MA 马中华 注释： key int 4byte
        if (key == null) {
            ByteUtils.writeVarint(-1, out);
        }
        // TODO_MA 马中华 注释： 如果有key的话，先写 key 长度，再写 key
        // TODO_MA 马中华 注释： keylenth, int, 4byte
        // TODO_MA 马中华 注释： key
        else {
        ...
        // TODO_MA 马中华 注释： value int 4byte
        if (value == null) {
            ByteUtils.writeVarint(-1, out);
        }
        // TODO_MA 马中华 注释： 如果有value的话， 先写value长度， 再写 value
        // TODO_MA 马中华 注释： valuelength, int，4byte
        ...
        // TODO_MA 马中华 注释： 写 header 的个数
        ByteUtils.writeVarint(headers.length, out);

        // TODO_MA 马中华 注释： 写 header
        for (Header header : headers) {
            String headerKey = header.key();
            if (headerKey == null) throw new IllegalArgumentException("Invalid null header key found in headers");

            // TODO_MA 马中华 注释： header key length int 4byte
            byte[] utf8Bytes = Utils.utf8(headerKey);
            ByteUtils.writeVarint(utf8Bytes.length, out);

            // TODO_MA 马中华 注释： header key
            out.write(utf8Bytes);

            // TODO_MA 马中华 注释： header value
            byte[] headerValue = header.value();
            // TODO_MA 马中华 注释： 如果 value 等于 null，则写个 -1 4byte
            if (headerValue == null) {
                ByteUtils.writeVarint(-1, out);
            } else {
                // TODO_MA 马中华 注释： 写 header value length， int 4byte
                ByteUtils.writeVarint(headerValue.length, out);
                // TODO_MA 马中华 注释： 写 header value
    ...
    public String toString() {
        return String.format("DefaultRecord(offset=%d, timestamp=%d, key=%d bytes, value=%d bytes)", offset, timestamp,
    ...
    public boolean equals(Object o) {
        ...
        return sizeInBytes == that.sizeInBytes && attributes == that.attributes && offset == that.offset && timestamp == that.timestamp && sequence == that.sequence && Objects.equals(
                key, that.key) && Objects.equals(value, that.value) && Arrays.equals(headers, that.headers);
    }
    ...
    public static DefaultRecord readFrom(DataInput input, long baseOffset, long baseTimestamp, int baseSequence,
        ...
        return readFrom(recordBuffer, totalSizeInBytes, sizeOfBodyInBytes, baseOffset, baseTimestamp, baseSequence, logAppendTime);
    }

    public static DefaultRecord readFrom(ByteBuffer buffer, long baseOffset, long baseTimestamp, int baseSequence,
                                         Long logAppendTime) {
        int sizeOfBodyInBytes = ByteUtils.readVarint(buffer);
        if (buffer.remaining() < sizeOfBodyInBytes) throw new InvalidRecordException(
                "Invalid record size: expected " + sizeOfBodyInBytes + " bytes in record payload, but instead the buffer has only " + buffer.remaining() + " remaining bytes.");

        int totalSizeInBytes = ByteUtils.sizeOfVarint(sizeOfBodyInBytes) + sizeOfBodyInBytes;
        return readFrom(buffer, totalSizeInBytes, sizeOfBodyInBytes, baseOffset, baseTimestamp, baseSequence, logAppendTime);
    }

    private static DefaultRecord readFrom(ByteBuffer buffer, int sizeInBytes, int sizeOfBodyInBytes, long baseOffset,
        ...
        try {
            ...
            int sequence = baseSequence >= 0 ? DefaultRecordBatch.incrementSequence(baseSequence,
                    offsetDelta) : RecordBatch.NO_SEQUENCE;
            ...
            if (numHeaders < 0) throw new InvalidRecordException("Found invalid number of record headers " + numHeaders);

            final Header[] headers;
            if (numHeaders == 0) headers = Record.EMPTY_HEADERS;
            else headers = readHeaders(buffer, numHeaders);

            // validate whether we have read all header bytes in the current record
            if (buffer.position() - recordStart != sizeOfBodyInBytes) throw new InvalidRecordException(
                    "Invalid record size: expected to read " + sizeOfBodyInBytes + " bytes in record payload, but instead read " + (buffer.position() - recordStart));
    ...
    public static PartialDefaultRecord readPartiallyFrom(DataInput input, byte[] skipArray, long baseOffset, long baseTimestamp,
                                                         int baseSequence, Long logAppendTime) throws IOException {
        int sizeOfBodyInBytes = ByteUtils.readVarint(input);
        int totalSizeInBytes = ByteUtils.sizeOfVarint(sizeOfBodyInBytes) + sizeOfBodyInBytes;

        return readPartiallyFrom(input, skipArray, totalSizeInBytes, sizeOfBodyInBytes, baseOffset, baseTimestamp, baseSequence,
                logAppendTime);
    }

    private static PartialDefaultRecord readPartiallyFrom(DataInput input, byte[] skipArray, int sizeInBytes, int sizeOfBodyInBytes,
                                                          long baseOffset, long baseTimestamp, int baseSequence,
                                                          Long logAppendTime) throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < numHeaders; i++) {
                ...
                if (headerKeySize < 0) throw new InvalidRecordException("Invalid negative header key size " + headerKeySize);

                // headerValueSize
                skipLengthDelimitedField(skipBuffer, input, bytesRemaining);
            }

            if (bytesRemaining.value > 0 || skipBuffer.remaining() > 0) throw new InvalidRecordException(
                    "Invalid record size: expected to read " + sizeOfBodyInBytes + " bytes in record payload, but there are still bytes remaining");
        ...
        while (true) {
            ...
            if (bytesToSkip < 0) {
                if (buffer.remaining() < 5 && bytesRemaining.value > 0) {
                    ...
                    if (sizeInBytes <= 0) return sizeInBytes;
                    else bytesToSkip = sizeInBytes;
    ...
    private static int sizeOf(int keySize, int valueSize, Header[] headers) {
        int size = 0;
        if (keySize < 0) size += NULL_VARINT_SIZE_BYTES;
        else size += ByteUtils.sizeOfVarint(keySize) + keySize;

        if (valueSize < 0) size += NULL_VARINT_SIZE_BYTES;
        else size += ByteUtils.sizeOfVarint(valueSize) + valueSize;
    ...
    static int recordSizeUpperBound(ByteBuffer key, ByteBuffer value, Header[] headers) {
        ...
        int valueSize = value == null ? -1 : value.remaining();
        // TODO_MA 马中华 注释： 21 + ?
        // TODO_MA 马中华 注释： 5 bytes length + 10 bytes timestamp + 5 bytes offset + 1 byte attributes
