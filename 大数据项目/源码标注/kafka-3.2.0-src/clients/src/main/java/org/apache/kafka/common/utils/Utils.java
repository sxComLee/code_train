...
public final class Utils {
    ...
    public static <T> Class<? extends T> loadClass(String klass, Class<T> base) throws ClassNotFoundException {
        return Class.forName(klass, true, Utils.getContextOrKafkaClassLoader())
                .asSubclass(base);
    }
    ...
    public static <T> T newParameterizedInstance(String className, Object... params) throws ClassNotFoundException {
        ...
        try {
            ...
            throw new ClassNotFoundException(
                    String.format("Failed to find " + "constructor with %s for %s", Utils.join(argTypes, ", "), className), e);
            ...
            throw new ClassNotFoundException(String.format("Unable to invoke " + "constructor of %s", className), e);
        }
    }
    ...
    public static boolean validHostPattern(String address) {
        return VALID_HOST_CHARACTERS.matcher(address)
                .matches();
    }
    ...
    public static String formatAddress(String host, Integer port) {
        return host.contains(":") ? "[" + host + "]:" + port // IPv6
                : host + ":" + port;
    }
     ...
     */
    public static <K, V> String mkString(Map<K, V> map, String begin, String end, String keyValueSeparator,
    ...
    public static Properties loadProps(String filename, List<String> onlyIncludeKeys) throws IOException {
        ...
        if (onlyIncludeKeys == null || onlyIncludeKeys.isEmpty()) return props;
    ...
    public static void delete(final File rootFile) throws IOException {
        ...
        Files.walkFileTree(rootFile.toPath(), new SimpleFileVisitor<Path>() {
            ...
            public FileVisitResult visitFileFailed(Path path, IOException exc) throws IOException {
                ...
                if (exc instanceof NoSuchFileException && path.toFile()
                        .equals(rootFile)) return FileVisitResult.TERMINATE;
                throw exc;
            }
    ...
    public static ClassLoader getContextOrKafkaClassLoader() {
        ...
        if (cl == null) return getKafkaClassLoader();
        else return cl;
    }
    ...
    public static void atomicMoveWithFallback(Path source, Path target, boolean needFlushParentDir) throws IOException {
        try {
            ...
            if (needFlushParentDir) {
                flushDir(target.toAbsolutePath()
                        .normalize()
                        .getParent());
            }
        }
    ...
    public static void closeQuietly(AutoCloseable closeable, String name) {
        if (closeable != null) {
            try {
                ...
                log.warn("Failed to close {} with type {}", name, closeable.getClass()
                        .getName(), t);
            }
        }
    }
    ...
    public static void closeQuietly(AutoCloseable closeable, String name, AtomicReference<Throwable> firstException) {
        if (closeable != null) {
            try {
                ...
                log.error("Failed to close {} with type {}", name, closeable.getClass()
                        .getName(), t);
            }
        }
    }
    ...
    public static void readFullyOrFail(FileChannel channel, ByteBuffer destinationBuffer, long position,
        ...
        if (destinationBuffer.hasRemaining()) {
            throw new EOFException(String.format(
                    "Failed to read `%s` from file channel `%s`. Expected to read %d bytes, " + "but reached end of file after reading %d bytes. Started read from position %d.",
    ...
    public static void readFully(InputStream inputStream, ByteBuffer destinationBuffer) throws IOException {
        if (!destinationBuffer.hasArray()) throw new IllegalArgumentException("destinationBuffer must be backed by an array");
     ...
     * @throws IOException If an I/O error occurs
     */
    public static long tryWriteTo(TransferableChannel destChannel, int position, int length,
    ...
    public static <K1, V1, K2, V2> Map<K2, V2> transformMap(Map<? extends K1, ? extends V1> map, Function<K1, K2> keyMapper,
    ...
    public static Map<String, Object> propsToMap(Properties properties) {
        ...
        for (Map.Entry<Object, Object> entry : properties.entrySet()) {
            if (entry.getKey() instanceof String) {
                ...
                throw new ConfigException(entry.getKey()
                        .toString(), entry.getValue(), "Key must be a string.");
            }
        }
        return map;
    }
    ...
    public static long getDateTime(String timestamp) throws ParseException, IllegalArgumentException {
        ...
        if (timestampParts.length < 2) {
            throw new ParseException("Error parsing timestamp. It does not contain a 'T' according to ISO8601 format",
                    timestamp.length());
        }
