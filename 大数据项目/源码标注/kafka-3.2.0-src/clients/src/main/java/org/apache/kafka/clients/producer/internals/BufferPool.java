...
public class BufferPool {
    ...
    // TODO_MA 马中华 注释： 内存队列，可以重复使用的 ByteBuffer 缓存池
    private final Deque<ByteBuffer> free;
    // TODO_MA 马中华 注释： 当我们构建了一个 ByteBuffer 从右边入队
    // TODO_MA 马中华 注释： 当我们使用完了一个 ByteBuffer 从左边入队

    private final Deque<Condition> waiters;

    /** Total available memory is the sum of nonPooledAvailableMemory and the number of byte buffers in free * poolableSize.  */
    // TODO_MA 马中华 注释： 专门用来给 大于 16kb 的数据单独申请内存使用的未池化内存
    ...
    public BufferPool(long memory, int poolableSize, Metrics metrics, Time time, String metricGrpName) {
        ...
        MetricName rateMetricName = metrics.metricName("bufferpool-wait-ratio", metricGrpName,
                "The fraction of time an appender waits for space allocation.");
        MetricName totalMetricName = metrics.metricName("bufferpool-wait-time-total", metricGrpName,
                "*Deprecated* The total time an appender waits for space allocation.");
        MetricName totalNsMetricName = metrics.metricName("bufferpool-wait-time-ns-total", metricGrpName,
                "The total time in nanoseconds an appender waits for space allocation.");

        Sensor bufferExhaustedRecordSensor = metrics.sensor("buffer-exhausted-records");
        MetricName bufferExhaustedRateMetricName = metrics.metricName("buffer-exhausted-rate", metricGrpName,
                "The average per-second number of record sends that are dropped due to buffer exhaustion");
        MetricName bufferExhaustedTotalMetricName = metrics.metricName("buffer-exhausted-total", metricGrpName,
    ...
    public ByteBuffer allocate(int size, long maxTimeToBlockMs) throws InterruptedException {
            ...
            if (this.nonPooledAvailableMemory + freeListSize >= size) {
                ...
                try {
                    ...
                    while (accumulated < size) {
                        ...
                        if (this.closed) throw new KafkaException("Producer closed while allocating memory");

                        if (waitingTimeElapsed) {
                            this.metrics.sensor("buffer-exhausted-records")
                                    .record();
                            throw new BufferExhaustedException(
                                    "Failed to allocate " + size + " bytes within the configured max blocking time " + maxTimeToBlockMs + " ms. Total memory: " + totalMemory() + " bytes. Available memory: " + availableMemory() + " bytes. Poolable size: " + poolableSize() + " bytes");
                        }
         ...
         *  注释：
         */
        if (buffer == null) return safeAllocateByteBuffer(size);
        else return buffer;
    }
    ...
    private ByteBuffer safeAllocateByteBuffer(int size) {
        ...
        try {
            ...
            if (error) {
                ...
                try {
                    this.nonPooledAvailableMemory += size;
                    if (!this.waiters.isEmpty()) this.waiters.peekFirst()
                            .signal();
                } finally {
                    this.lock.unlock();
                }
    ...
    private void freeUp(int size) {
        while (!this.free.isEmpty() && this.nonPooledAvailableMemory < size) this.nonPooledAvailableMemory += this.free.pollLast()
                .capacity();
    }
