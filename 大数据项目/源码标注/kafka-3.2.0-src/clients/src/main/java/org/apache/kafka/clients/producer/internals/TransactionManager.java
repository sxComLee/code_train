...
public class TransactionManager {
    ...
    private static class TopicPartitionBookkeeper {
        ...
        private TopicPartitionEntry getPartition(TopicPartition topicPartition) {
            ...
            if (ent == null) throw new IllegalStateException(
                    "Trying to get the sequence number for " + topicPartition + ", but the sequence number was never set for this partition.");
            return ent;
        }
    ...
    private static class TopicPartitionEntry {
        ...
        private static final Comparator<ProducerBatch> PRODUCER_BATCH_COMPARATOR = Comparator.comparingLong(
                        ProducerBatch::producerId)
    ...
    public TransactionManager(final LogContext logContext, final String transactionalId, final int transactionTimeoutMs,
                              final long retryBackoffMs, final ApiVersions apiVersions) {
        this.producerIdAndEpoch = ProducerIdAndEpoch.NONE;
    ...
    synchronized TransactionalRequestResult initializeTransactions(ProducerIdAndEpoch producerIdAndEpoch) {
        ...
        return handleCachedTransactionRequestResult(() -> {
            ...
            if (!isEpochBump) {
                ...
                log.info("Invoking InitProducerId with current producer ID and epoch {} in order to bump the epoch",
                        producerIdAndEpoch);
            }
            InitProducerIdRequestData requestData = new InitProducerIdRequestData().setTransactionalId(transactionalId)
    ...
    private TransactionalRequestResult beginCompletingTransaction(TransactionResult transactionResult) {
        ...
        if (!(lastError instanceof InvalidPidMappingException)) {
            EndTxnRequest.Builder builder = new EndTxnRequest.Builder(new EndTxnRequestData().setTransactionalId(transactionalId)
                    .setProducerId(producerIdAndEpoch.producerId)
                    .setProducerEpoch(producerIdAndEpoch.epoch)
    ...
    public synchronized TransactionalRequestResult sendOffsetsToTransaction(final Map<TopicPartition, OffsetAndMetadata> offsets,
        ...
        AddOffsetsToTxnRequest.Builder builder = new AddOffsetsToTxnRequest.Builder(
                new AddOffsetsToTxnRequestData().setTransactionalId(transactionalId)
                        .setProducerId(producerIdAndEpoch.producerId)
                        .setProducerEpoch(producerIdAndEpoch.epoch)
    ...
    synchronized void transitionToAbortableError(RuntimeException exception) {
        if (currentState == State.ABORTING_TRANSACTION) {
            log.debug(
                    "Skipping transition to abortable error state since the transaction is already being " + "aborted. Underlying exception: ",
                    exception);
            return;
        }
    ...
    synchronized public void maybeUpdateProducerIdAndEpoch(TopicPartition topicPartition) {
        if (hasStaleProducerIdAndEpoch(topicPartition) && !hasInflightBatches(topicPartition)) {
            ...
            log.debug("ProducerId of partition {} set to {} with epoch {}. Reinitialize sequence at beginning.", topicPartition,
    ...
    private void resetIdempotentProducerId() {
        if (isTransactional()) throw new IllegalStateException(
                "Cannot reset producer state for a transactional producer. " + "You must either abort the ongoing transaction or reinitialize the transactional producer instead");
    ...
    synchronized void bumpIdempotentEpochAndResetIdIfNeeded() {
        if (!isTransactional()) {
            ...
            if (currentState != State.INITIALIZING && !hasProducerId()) {
                transitionTo(State.INITIALIZING);
                InitProducerIdRequestData requestData = new InitProducerIdRequestData().setTransactionalId(null)
    ...
    synchronized void addInFlightBatch(ProducerBatch batch) {
        if (!batch.hasSequence()) throw new IllegalStateException(
    ...
    synchronized int firstInFlightSequence(TopicPartition topicPartition) {
        ...
        if (inflightBatches.isEmpty()) return RecordBatch.NO_SEQUENCE;
        else return inflightBatches.first()
                .baseSequence();
    }
    ...
    public synchronized void handleCompletedBatch(ProducerBatch batch, ProduceResponse.PartitionResponse response) {
        ...
        log.debug("ProducerId: {}; Set last ack'd sequence number for topic-partition {} to {}", batch.producerId(),
                batch.topicPartition, lastAckedSequence);
    ...
    synchronized void handleFailedBatch(ProducerBatch batch, RuntimeException exception, boolean adjustSequenceNumbers) {
        ...
        if (hasFatalError()) {
            log.debug(
                    "Ignoring batch {} with producer id {}, epoch {}, and sequence number {} " + "since the producer is already in fatal error state",
                    batch, batch.producerId(), batch.producerEpoch(), batch.baseSequence(), exception);
            return;
        }
    ...
    private void adjustSequencesDueToFailedBatch(ProducerBatch batch) {
        if (!topicPartitionBookkeeper.contains(batch.topicPartition))
            ...
            return;
        log.debug("producerId: {}, send to partition {} failed fatally. Reducing future sequence numbers by {}", batch.producerId(),
                batch.topicPartition, batch.recordCount);
        int currentSequence = sequenceNumber(batch.topicPartition);
        currentSequence -= batch.recordCount;
        if (currentSequence < 0) throw new IllegalStateException(
                "Sequence number for partition " + batch.topicPartition + " is going to become negative: " + currentSequence);
        ...
        topicPartitionBookkeeper.getPartition(batch.topicPartition)
                .resetSequenceNumbers(inFlightBatch -> {
                    if (inFlightBatch.baseSequence() < batch.baseSequence()) return;

                    int newSequence = inFlightBatch.baseSequence() - batch.recordCount;
                    if (newSequence < 0) throw new IllegalStateException(
                            "Sequence number for batch with sequence " + inFlightBatch.baseSequence() + " for partition " + batch.topicPartition + " is going to become negative: " + newSequence);

                    log.info("Resetting sequence number of batch with current sequence {} for partition {} to {}",
                            inFlightBatch.baseSequence(), batch.topicPartition, newSequence);
                    inFlightBatch.resetProducerState(
                            new ProducerIdAndEpoch(inFlightBatch.producerId(), inFlightBatch.producerEpoch()), newSequence,
    ...
    synchronized void maybeResolveSequences() {
        for (Iterator<TopicPartition> iter = partitionsWithUnresolvedSequences.keySet()
            ...
            if (!hasInflightBatches(topicPartition)) {
                ...
                if (isNextSequence(topicPartition, sequenceNumber(topicPartition))) {
                    ...
                    if (isTransactional()) {
                        ...
                        log.info(
                                "No inflight batches remaining for {}, last ack'd sequence for partition is {}, next sequence is {}. " + "Going to bump epoch and reset sequence numbers.",
                                topicPartition, lastAckedSequence(topicPartition).orElse(NO_LAST_ACKED_SEQUENCE_NUMBER),
    ...
    synchronized TxnRequestHandler nextRequest(boolean hasIncompleteBatches) {
        ...
        if (nextRequestHandler == null) return null;
        ...
        if (maybeTerminateRequestWithError(nextRequestHandler)) {
            log.trace("Not sending transactional request {} because we are in an error state", nextRequestHandler.requestBuilder());
            return null;
        }
        ...
        if (nextRequestHandler != null) log.trace("Request {} dequeued for sending", nextRequestHandler.requestBuilder());
    ...
    synchronized boolean canRetry(ProduceResponse.PartitionResponse response, ProducerBatch batch) {
        ...
        if (error == Errors.UNKNOWN_PRODUCER_ID) {
            ...
            if (!hasUnresolvedSequence(batch.topicPartition) && (batch.sequenceHasBeenReset() || !isNextSequence(
                ...
                if (!hasUnresolvedSequence(batch.topicPartition) || isNextSequenceForUnresolvedPartition(batch.topicPartition,
                        batch.baseSequence())) {
                    requestEpochBumpForPartition(batch.topicPartition);
                }
                return true;
    ...
    void handleCoordinatorReady() {
        NodeApiVersions nodeApiVersions = transactionCoordinator != null ? apiVersions.get(
                transactionCoordinator.idString()) : null;
    ...
    private void transitionTo(State target, RuntimeException error) {
        ...
        if (target == State.FATAL_ERROR || target == State.ABORTABLE_ERROR) {
            if (error == null) throw new IllegalArgumentException("Cannot transition to " + target + " with a null exception");
            lastError = error;
        } else {
            lastError = null;
        }

        if (lastError != null) log.debug("Transition from state {} to error state {}", currentState, target, lastError);
        ...
        if (hasError()) {
            ...
            if (lastError instanceof ProducerFencedException) {
                ...
                throw new InvalidProducerEpochException(
                        "Producer with transactionalId '" + transactionalId + "' and " + producerIdAndEpoch + " attempted to produce with an old epoch");
            } else {
    ...
    private void lookupCoordinator(FindCoordinatorRequest.CoordinatorType type, String coordinatorKey) {
        ...
        FindCoordinatorRequestData data = new FindCoordinatorRequestData().setKeyType(type.id())
    ...
    private TxnOffsetCommitHandler txnOffsetCommitHandler(TransactionalRequestResult result,
        ...
        final TxnOffsetCommitRequest.Builder builder = new TxnOffsetCommitRequest.Builder(transactionalId, groupMetadata.groupId(),
                producerIdAndEpoch.producerId, producerIdAndEpoch.epoch, pendingTxnOffsetCommits, groupMetadata.memberId(),
                groupMetadata.generationId(), groupMetadata.groupInstanceId());
    ...
    private void throwIfPendingState(String operation) {
        if (pendingTransition != null) {
            if (pendingTransition.result.isAcked()) {
                ...
                throw new IllegalStateException(
                        "Cannot attempt operation `" + operation + "` " + "because the previous call to `" + pendingTransition.operation + "` " + "timed out and must be retried");
            }
        }
    }

    private TransactionalRequestResult handleCachedTransactionRequestResult(
            Supplier<TransactionalRequestResult> transactionalRequestResultSupplier, State nextState, String operation) {
        ensureTransactional();
    ...
    abstract class TxnRequestHandler implements RequestCompletionHandler {
        ...
        public void onComplete(ClientResponse response) {
            if (response.requestHeader()
                    .correlationId() != inFlightRequestCorrelationId) {
                ...
                if (response.wasDisconnected()) {
                    ...
                    if (this.needsCoordinator()) lookupCoordinator(this.coordinatorType(), this.coordinatorKey());
                    reenqueue();
                } else if (response.versionMismatch() != null) {
                    fatalError(response.versionMismatch());
                } else if (response.hasResponse()) {
                    log.trace("Received transactional response {} for request {}", response.responseBody(), requestBuilder());
                    synchronized (TransactionManager.this) {
    ...
    private class InitProducerIdHandler extends TxnRequestHandler {
        ...
        public void handleResponse(AbstractResponse response) {
            ...
            if (error == Errors.NONE) {
                ProducerIdAndEpoch producerIdAndEpoch = new ProducerIdAndEpoch(initProducerIdResponse.data()
                        .producerId(), initProducerIdResponse.data()
                        .producerEpoch());
    ...
    private class AddPartitionsToTxnHandler extends TxnRequestHandler {
        ...
        public void handleResponse(AbstractResponse response) {
            ...
            for (Map.Entry<TopicPartition, Errors> topicPartitionErrorEntry : errors.entrySet()) {
                ...
                if (error == Errors.NONE) {
                    ...
                    log.debug(
                            "Did not attempt to add partition {} to transaction because other partitions in the " + "batch had errors.",
                            topicPartition);
        ...
        private void maybeOverrideRetryBackoffMs() {
            ...
            if (partitionsInTransaction.isEmpty()) this.retryBackoffMs = ADD_PARTITIONS_RETRY_BACKOFF_MS;
        }
    }
    ...
    private class FindCoordinatorHandler extends TxnRequestHandler {
        ...
        @Override
        public void handleResponse(AbstractResponse response) {
            CoordinatorType coordinatorType = CoordinatorType.forId(builder.data()
                    .keyType());
            ...
            if (coordinators.size() != 1) {
                ...
                fatalError(new IllegalStateException(
            ...
            String key = coordinatorData.key() == null ? builder.data()
                    .key() : coordinatorData.key();
            ...
            if (error == Errors.NONE) {
                ...
                result.done();
                log.info("Discovered {} coordinator {}", coordinatorType.toString()
                        .toLowerCase(Locale.ROOT), node);
                ...
                fatalError(new KafkaException(
                        String.format("Could not find a coordinator with type %s with key %s due to " + "unexpected error: %s",
                                coordinatorType, key, coordinatorData.errorMessage())));
            }
        }
    }
    ...
    private class AddOffsetsToTxnHandler extends TxnRequestHandler {
        ...
        public void handleResponse(AbstractResponse response) {
            ...
            Errors error = Errors.forCode(addOffsetsToTxnResponse.data()
                    .errorCode());
    ...
    private class TxnOffsetCommitHandler extends TxnRequestHandler {
        ...
        public void handleResponse(AbstractResponse response) {
            ...
            log.debug("Received TxnOffsetCommit response for consumer group {}: {}", builder.data.groupId(), errors);
            ...
            for (Map.Entry<TopicPartition, Errors> entry : errors.entrySet()) {
                ...
                if (error == Errors.NONE) {
                ...
                } else if (error == Errors.UNKNOWN_TOPIC_OR_PARTITION || error == Errors.COORDINATOR_LOAD_IN_PROGRESS) {
                    ...
                    abortableError(new CommitFailedException(
                            "Transaction offset Commit failed " + "due to consumer group metadata mismatch: " + error.exception()
                                    .getMessage()));
                    break;
    ...
    private boolean isFatalException(Errors error) {
        return error == Errors.TRANSACTIONAL_ID_AUTHORIZATION_FAILED || error == Errors.INVALID_PRODUCER_EPOCH || error == Errors.PRODUCER_FENCED || error == Errors.UNSUPPORTED_FOR_MESSAGE_FORMAT;
    }
    ...
    private static final class PendingStateTransition {
        ...
        private PendingStateTransition(TransactionalRequestResult result, State state, String operation) {
            this.result = result;
            this.state = state;
