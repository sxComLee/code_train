...
public class FileRecords extends AbstractRecords implements Closeable {
     ...
     *  注释： 通过 NIO 提供的 FileChannel 来写文件。这里会用到 零拷贝技术
     *  FileChannel 文件操作类，支持 零拷贝的， 往 xxxx.log 写数据
     *  写索引不是通过这个 FileChannel channel
     */
    ...
    FileRecords(File file, FileChannel channel, int start, int end, boolean isSlice) throws IOException {
        ...
        if (isSlice) {
            ...
            if (channel.size() > Integer.MAX_VALUE) throw new KafkaException(
                    "The size of segment " + file + " (" + channel.size() + ") is larger than the maximum allowed segment size of " + Integer.MAX_VALUE);
    ...
    private int availableBytes(int position, int size) {
        ...
        if (position < 0) throw new IllegalArgumentException("Invalid position: " + position + " in read from " + this);
        ...
        if (size < 0) throw new IllegalArgumentException("Invalid size: " + size + " in read from " + this);

        int end = this.start + position + size;
        // Handle integer overflow or if end is beyond the end of the file
        if (end < 0 || end > start + currentSizeInBytes) end = this.start + currentSizeInBytes;
    ...
    public int append(MemoryRecords records) throws IOException {
        if (records.sizeInBytes() > Integer.MAX_VALUE - size.get()) throw new IllegalArgumentException(
                "Append of size " + records.sizeInBytes() + " bytes is too large for segment with current file position at " + size.get());

        /***********************************************************************
         * TODO_MA 马中华 微信：zhongqi2513 博客：https://blog.csdn.net/zhongqi2513
         *  注释： 将 MemoryRecords 也就是要写入的数据，写到 FileChannel 中
         */
    ...
    public int truncateTo(int targetSize) throws IOException {
        ...
        if (targetSize > originalSize || targetSize < 0) throw new KafkaException(
                "Attempt to truncate log segment " + file + " to " + targetSize + " bytes failed, " + " size of this log segment is " + originalSize + " bytes.");
    ...
    public ConvertedRecords<? extends Records> downConvert(byte toMagic, long firstOffset, Time time) {
        ...
        if (convertedRecords.recordConversionStats()
                .numRecordsConverted() == 0) {
    ...
    public long writeTo(TransferableChannel destChannel, long offset, int length) throws IOException {
        ...
        if (newSize < oldSize) throw new KafkaException(
                String.format("Size of FileRecords %s has been truncated during write: old size %d, new size %d",
    ...
    private AbstractIterator<FileChannelRecordBatch> batchIterator(int start) {
        final int end;
        if (isSlice) end = this.end;
        else end = this.sizeInBytes();
    ...
    public static FileRecords open(File file, boolean mutable, boolean fileAlreadyExists, int initFileSize,
        ...
        // TODO_MA 马中华 注释： 构建 FileChannel 用来写入数据到 Disk SegmentFile
     ...
     */
    private static FileChannel openChannel(File file, boolean mutable, boolean fileAlreadyExists, int initFileSize,
                                           boolean preallocate) throws IOException {
        if (mutable) {
