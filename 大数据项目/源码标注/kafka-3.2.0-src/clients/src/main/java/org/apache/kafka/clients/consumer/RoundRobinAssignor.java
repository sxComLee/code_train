...
public class RoundRobinAssignor extends AbstractPartitionAssignor {
    ...
    public Map<String, List<TopicPartition>> assign(Map<String, Integer> partitionsPerTopic,
        ...
        for (Map.Entry<String, Subscription> memberSubscription : subscriptions.entrySet()) {
            ...
            memberInfoList.add(new MemberInfo(memberSubscription.getKey(), memberSubscription.getValue()
                    .groupInstanceId()));
        }
        ...
        for (TopicPartition partition : allPartitionsSorted(partitionsPerTopic, subscriptions)) {
            ...
            while (!subscriptions.get(assigner.peek().memberId)
                    .topics()
                    .contains(topic)) assigner.next();
            assignment.get(assigner.next().memberId)
                    .add(partition);
        }
        return assignment;
    }
