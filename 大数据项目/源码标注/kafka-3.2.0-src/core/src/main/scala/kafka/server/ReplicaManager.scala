...
class ReplicaManager(val config: KafkaConfig,
    ...
    // TODO_MA 马中华 注释： ZK 客户端
    val zkClient: Option[KafkaZkClient] = None,
    // TODO_MA 马中华 注释： 4 个时间轮用来管理四类不同的消息
    ...
    // TODO_MA 马中华 注释： Controller Epoch 信息
    @volatile private[server] var controllerEpoch: Int = KafkaController.InitialControllerEpoch

    // TODO_MA 马中华 注释： BrokerID
    protected val localBrokerId = config.brokerId

    /**
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： 存储了当前 Broker 上保存的所有的 Partition 的信息
     */
    ...
    /**
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： 创建 ReplicaFetcherManager
     *  ReplicaFetcherManager 内部维护了多个（num.replica.fetchers） ReplicaFetcherThread 线程用来向 Leader Replica 发送
     *  FetchRequest 请求来获取消息，实现 Follower Replia 和 Leader Replica 的同步。
     *  一个附带知识：Leader KafkaController 会发送 LeaderAndIsrRequest 给 Borker 用于 Replica 的角色切换
     *  Broker 根据相关信息就可以将 每个 Replica 的状态更新成 Leader 或者 Follower
     *  另外一个附带的知识： ReplicaFetcherThread 的职责就是 封装 FetchRequest 请求发送给给 Leader Replica 所在的 Broker
     */
    val replicaFetcherManager = createReplicaFetcherManager(metrics, time, threadNamePrefix, quotaManagers.follower)

    // TODO_MA 马中华 注释： LogManager 负责完成数据的写入
    ...
    // TODO_MA 马中华 注释： 用于缓存 Log 和 OffsetCheckpint 之间的关系
    @volatile
    private[server] var highWatermarkCheckpoints: Map[String, OffsetCheckpointFile] = logManager.liveLogDirs.map(dir =>
    ...
    def startup(): Unit = {
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 启动周期性任务 shutdownIdleReplicaAlterLogDirsThread()
         *  关闭和移除闲置的 ReplicaFetcherThread 线程
         *  如果之前有一个 topica_partition01 的分区的一个副本在当前节点，并且是 follower
         *  这个 follower replica 就需要保持和 topica_partition01 的leader replica 保持同步
         *  我们会启动一个叫做 ReplicaFetcherThread 的线程来完成这个同步的工作
         *  万一这个  topica_partition01 的这个 follower replica 不在这个节点 Broker 了。
         *  ReplicaFetcherThread 服务的分区数量，如果小于 1， 就闲置了。 需要关闭
         */
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 创建和启动 LogDirFailureHandler
         *  不管是哪个 broker 节点，肯定都需要指定一个 数据存储目录 log.dirs
         *  为了提高 Broker 的工作效率和 磁盘的吞吐， 会配置 log.dirs 使用多块磁盘
         *  配置方式：
         *  log.dirs = dir1,dir2,dir3    这3个文件夹就是3个不同的磁盘，  hdfs,zookeeper
         *  万一出现有一块磁盘两块磁盘失败（异常）LogDirFailureHandler 完成这个磁盘异常的处理
         *  -
         *  集群的硬件选型的： HDD SSD
         */
    ...
    /**
     * // TODO_MA 马中华 注释： 如果客户端发送 PRODUCE 类型请求 给 当前Broker
     * // TODO_MA 马中华 注释： 首先是由 KafkaApis 组件的 handle() 处理之后转交给 ReplicaManager
     * // TODO_MA 马中华 注释： 那么调用 ReplicaManager 的 appendRecords 完成数据写入
    ...
    def appendRecords(timeout: Long,
        ...
        if (isValidRequiredAcks(requiredAcks)) {
            ...
            /**
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： entriesPerPartition 就是要执行追加写的数据
             *  1、requiredAcks, 只要 leader 写成功就返回
             *  2、必须 Leader 和 所有的 Follower 都写成功才返回
             */
    ...
    private def delayedProduceRequestRequired(requiredAcks: Short,
        entriesPerPartition: Map[TopicPartition, MemoryRecords],
        localProduceResults: Map[TopicPartition, LogAppendResult]
    ): Boolean = {
        requiredAcks == -1 &&
    ...
    private def appendToLocalLog(internalTopicsAllowed: Boolean,
        ...
        entriesPerPartition.map { case (topicPartition, records) =>
            ...
            // TODO_MA 马中华 注释： 如果是往 系统 Topic 插入数据的话，这是不允许的，所以抛异常
            ...
            // TODO_MA 马中华 注释： 正常处理
            // TODO_MA 马中华 注释： 下面这段代码执行一次，表示往一个分区，写一个批次
            else {
                try {

                    // TODO_MA 马中华 注释： 获取 Partition
                    val partition = getPartitionOrException(topicPartition)

                    // TODO_MA 马中华 注释： 将数据写入 Leader，注意携带了 requiredAcks，表示是否需要后去反馈返回给客户端
    ...
    /**
     * // TODO_MA 马中华 注释： 如果 ReplicaMaanger 处理的是 拉取数据的请求，则调用这个方法，从 Log 中读取数据
     ...
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： 核心分为 4 个步骤：
     *  1、首先解析 Leader KafkaController 发送的 LeaderAndIsrRequest 获取那些 Replica 需要切换成 Leader，那些需要切换成 Follower
     *  2、遍历 partitionsToBeLeader 集合，让集合中的每个 Replia 的状态都切换成 Leader
     *  3、遍历 partitionsToBeFollower 集合，让集合中的每个 Replia 的状态都切换成 Follower
     *  4、对于发生了状态变更的 Replica 需要进行回调。入口方法是：onLeadershipChange(partitionsToBeLeader, partitionsToBeFollower)
     */
                ...
                if (leaderAndIsrRequest.controllerEpoch < controllerEpoch) {
                    ...
                    requestPartitionStates.foreach { partitionState =>
                        ...
                        val partitionOpt = getPartition(topicPartition) match {
                            ...
                            // TODO_MA 马中华 注释： 离线 Partition 属于异常 Partition
                            case HostedPartition.Offline =>
                            ...
                            // TODO_MA 马中华 注释： Online Partition 是正常的
                            case HostedPartition.Online(partition) =>
                                Some(partition)
    ...
    private def makeLeaders(controllerId: Int,
        ...
        try {
            ...
            // TODO_MA 马中华 注释： 遍历每个需要切换状态为 Leader 的 Replica/Partition
    ...
    private def makeFollowers(controllerId: Int,
        ...
        try {
            /**
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 遍历每个需要切换成 Follower 的 Partition Replica
             */
            partitionStates.forKeyValue { (partition, partitionState) =>
            ...
            /**
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 3、 给成为 Follower 的 Replica 移除掉 Fetcher 组件
             */
            ...
            /**
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 5、Clear the produce and fetch requests in the purgatory
             *  移除时间轮中管理的 PRODUCE 和 FETCH 请求
             */
            ...
            if (isShuttingDown.get()) {
                ...
                /**
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 6、对于成为 Leader 的 Replica，我们给它打开 Fetcher
                 */
    ...
    private def updateFollowerFetchState(followerId: Int,
        readResults: Seq[(TopicIdPartition, LogReadResult)]
    ): Seq[(TopicIdPartition, LogReadResult)] = {
