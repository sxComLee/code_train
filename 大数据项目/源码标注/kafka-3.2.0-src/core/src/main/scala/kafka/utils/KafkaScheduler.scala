...
trait Scheduler {
     ...
     *
     *         // TODO_MA 马中华 注释： 从现在开始，过 delay 时间之后，开始每隔 period 时长执行名字叫做 name 的任务 fun
     *         // TODO_MA 马中华 注释： 在java中，一般表示一个任务都是一个 Runnable， 一个任务就是一个方法
     *         // TODO_MA 马中华 注释：
     */
 ...
 *               // TODO_MA 马中华 注释： Scheduler 接口中的最重要的方法，就是 schedule()
 */
@threadsafe
class KafkaScheduler(val threads: Int,
    ...
    override def startup(): Unit = {
        ...
        this synchronized {
            ...
            // TODO_MA 马中华 注释： 调度线程池
            // TODO_MA 马中华 注释： Timer 调度，
    ...
    def schedule(name: String, fun: () => Unit, delay: Long, period: Long, unit: TimeUnit): ScheduledFuture[_] = {
        ...
        this synchronized {
            if (isStarted) {
                ...
                // TODO_MA 马中华 注释： 如果 period 大于 0，则是指定的 周期性调度
                if (period >= 0)
