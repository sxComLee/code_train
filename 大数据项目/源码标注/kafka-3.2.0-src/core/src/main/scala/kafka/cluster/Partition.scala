...
class Partition(val topicPartition: TopicPartition,
    ...
    // TODO_MA 马中华 注释： UnifiedLog 的一个对象，就代表了一个 Broker 里面的一个 数据存储目录里面的一个 分区目录
    ...
    def leaderLogIfLocal: Option[UnifiedLog] = {
        ...
        // TODO_MA 马中华 注释： 判断这个分区是不是leader分区
    ...
    def appendRecordsToLeader(records: MemoryRecords, origin: AppendOrigin, requiredAcks: Int,
        ...
        val (info, leaderHWIncremented) = inReadLock(leaderIsrUpdateLock) {
            ...
            // TODO_MA 马中华 注释： 拿出来 Leader Log
            // TODO_MA 马中华 注释： leaderLogIfLocal() 方法的逻辑，当前这个 replica 对应的分区的 leader replica 的 Broker ID 拿出来和当前这个节点的 Broker 作比较
            // TODO_MA 马中华 注释： 和当前这个 Broker ID 作比较
            leaderLogIfLocal match {

                // TODO_MA 马中华 注释： 如果 Leader Replica 就是在本地
                case Some(leaderLog) =>
                    val minIsr = leaderLog.config.minInSyncReplicas
                     ...
                     *  注释： 调用 Leader 追加写数据
                     *  records 就是要写进去的数据
                     *  leaderLog = UnifiedLog 代表的是一个分区，对应的管理了一个分区目录
                     */
    ...
    def fetchOffsetSnapshot(currentLeaderEpoch: Optional[Integer],
        fetchOnlyFromLeader: Boolean
    ): LogOffsetSnapshot = inReadLock(leaderIsrUpdateLock) {
        // decide whether to only fetch from leader
        val localLog = localLogWithEpochOrException(currentLeaderEpoch, fetchOnlyFromLeader)
    ...
    def legacyFetchOffsetsForTimestamp(timestamp: Long,
        ...
        fetchOnlyFromLeader: Boolean
    ): Seq[Long] = inReadLock(leaderIsrUpdateLock) {
