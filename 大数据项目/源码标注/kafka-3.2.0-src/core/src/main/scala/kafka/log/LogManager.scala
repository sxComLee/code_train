...
/**
 * // TODO_MA 马中华 注释： kafka 日志管理子系统的入口点。
 * The entry point to the kafka log management subsystem.
 *
 * // TODO_MA 马中华 注释： 日志管理器负责日志的创建、检索和清理。所有读取和写入操作都委托给各个日志实例。
 * The log manager is responsible for log creation, retrieval, and cleaning.
 * All read and write operations are delegated to the individual log instances.
 *
 * // TODO_MA 马中华 注释： 日志管理器在一个或多个目录中维护日志。
 * // TODO_MA 马中华 注释： 在日志最少的数据目录中创建新日志。事后不会尝试移动分区或根据大小或 IO 速率进行平衡。
 * The log manager maintains logs in one or more directories. New logs are created in the data directory
 * with the fewest logs. No attempt is made to move partitions after the fact or balance based on
 * size or I/O rate.
 *
 * // TODO_MA 马中华 注释： 后台线程通过定期截断多余的日志段来处理日志保留。
...
class LogManager(logDirs: Seq[File],
    ...
    @volatile
    private var logStartOffsetCheckpoints = liveLogDirs.map(dir =>
    ...
    private[log] def loadLog(logDir: File,
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 构造一个 UnifiedLog 对象用来管理这个 LogDir
         */
        val log = UnifiedLog(
            dir = logDir,
            config = config,
        ...
        // TODO_MA 马中华 注释： 如果发现日志目录是 -delete 结尾，则加入到待删除集合中等待删除
    ...
    private[log] def loadLogs(defaultConfig: LogConfig, topicConfigOverrides: Map[String, LogConfig]): Unit = {
        ...
        // TODO_MA 马中华 注释： 注意这个集合： 每个 Log 日志目录，都有一个自己的线程池
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 遍历每个 日志存储目录。进行启动时 日志目录的 读取
         *  这个地方遍历的是 Broker 设置的多个 数据存储目录，这样的每个目录下包含了多个 分区目录
         */
        for (dir <- liveLogDirs) {
            ...
            try {
                ...
                // TODO_MA 马中华 注释： 针对每个 Log 日志目录，初始化一个线程池， 加入到集合中
                // TODO_MA 马中华 注释： 这个拥有固定线程数量的线程池的 线程的数量 = num.recovery.threads.per.data.dir
                val pool = Executors.newFixedThreadPool(numRecoveryThreadsPerDataDir,
                    KafkaThread.nonDaemon(s"log-recovery-$logDirAbsolutePath", _))
                threadPools.append(pool)

                // TODO_MA 马中华 注释： Broker 在关闭的时候，会创建 .kafka_cleanshutdown 文件
                // TODO_MA 马中华 注释： 如果存在 .kafka_cleanshutdown 文件，则先删除
                ...
                /**
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 读取 日志存储目录下的 recovery-point-offset-checkpoint 文件中的 recovery-point 内容
                 */
                ...
                /**
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 读取 日志存储目录下的 log-start-offset-checkpoint 文件的 LSO 内容
                 */
                ...
                /**
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 进行过滤得到 要执行加载的 合法的 Log Dir 列表
                 */
                ...
                /**
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 遍历每个 Log Dir 执行加载
                 *  这个地方遍历得到的 logDir 是数据存储目录下的每个分区目录
                 */
                val jobsForDir = logsToLoad.map { logDir =>

                    // TODO_MA 马中华 注释： 一个日志存储目录，构造一个任务扫描任务，加入到线程池
                    val runnable: Runnable = () => {
                        try {
                            ...
                            /**
                             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                             *  注释： 核心方法，加载 LogFile 信息， 构造 UnifiedLog 返回
                             *  UnifiedLog 是用来管理 LogFile 的抽象对象
                             */
                ...
                /**
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 将每个 LogDir 对应的日志扫描任务 ，提交到线程池中执行
                 */
    ...
    def startup(topicNames: Set[String]): Unit = {
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 启动 LogManager 的内部，其实就是 加载 log 信息，并且启动一堆后台服务
         */
    ...
    private[log] def startupWithConfigOverrides(defaultConfig: LogConfig, topicConfigOverrides: Map[String, LogConfig]): Unit = {
         ...
         *  注释： 启动一堆后台定时调度服务
         *  1、调度 cleanupLogs() 任务
         *  2、调度 flushDirtyLogs() 任务
         *  3、调度 checkpointLogRecoveryOffsets() 任务
         *  4、调度 checkpointLogStartOffsets() 任务
         *  5、调度 deleteLogs() 任务
         */
        if (scheduler != null) {

            // TODO_MA 马中华 注释： 调度 cleanupLogs() 任务
            // TODO_MA 马中华 注释： 用来删除过期的 LogSegment，或者截断最旧的 LogSegment 用来保证 Log 的大小
            ...
            // TODO_MA 马中华 注释： 调度 flushDirtyLogs() 任务
            // TODO_MA 马中华 注释： 用来周期性的对未 flush 的数据执行 flush 到 Disk
            ...
            // TODO_MA 马中华 注释： 调度 checkpointLogRecoveryOffsets() 任务
            // TODO_MA 马中华 注释： 用来定时将每个 Log 的 recoveryPoint 写入到文本文件 recovery-point-offset-checkpoint 中
            ...
            // TODO_MA 马中华 注释： 调度 checkpointLogStartOffsets() 任务
            // TODO_MA 马中华 注释： 用来周期性的将每个 Log 的 LSO 写到 log 目录中的一个文本文件 log-start-offset-checkpoint 中
            ...
            // TODO_MA 马中华 注释： 调度 deleteLogs() 任务
            // TODO_MA 马中华 注释： 用来周期性的删除标记上了 -delete 的分区目录。如果一个 Topic 要删除的话，则它的分区目录都会标记上 -delete
    ...
    /**
     * // TODO_MA 马中华 注释： 删除标记上了 -delete 的分区
    ...
    private def deleteLogs(): Unit = {
        ...
        // TODO_MA 马中华 注释： file.delete.delay.ms
        ...
        try {
            ...
            while ( {
                ...
                // TODO_MA 马中华 注释： 遍历拿到一个待删除的 Log
                val (removedLog, _) = logsToBeDeleted.take()
                if (removedLog != null) {
                    try {
                        /**
                         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                         *  注释： 执行 xxx-delete 分区目录的删除
                         */
                        removedLog.delete()
...
object LogManager {
    ...
    def apply(config: KafkaConfig,
        ...
        // TODO_MA 马中华 注释： 解析得到 log 相关的各种配置
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 负责日志管理。 可以认为 LogManager 就是 Broker 中负责处理 Log 相关的一个子系统
         *  包括日志的读写，检索，过期清理等等工作
         */
        new LogManager(logDirs = config.logDirs.map(new File(_).getAbsoluteFile),
            ...
            // TODO_MA 马中华 注释： log.flush.scheduler.interval.ms
            // TODO_MA 马中华 注释： The frequency in ms that the log flusher checks whether any log needs to be flushed to disk
            flushCheckMs = config.logFlushSchedulerIntervalMs,
            // TODO_MA 马中华 注释： log.flush.offset.checkpoint.interval.ms
            // TODO_MA 马中华 注释： The frequency with which we update the persistent record of the last flush which acts as the log recovery point
            flushRecoveryOffsetCheckpointMs = config.logFlushOffsetCheckpointIntervalMs,
            // TODO_MA 马中华 注释： log.flush.start.offset.checkpoint.interval.ms
            // TODO_MA 马中华 注释： The frequency with which we update the persistent record of log start offset
            flushStartOffsetCheckpointMs = config.logFlushStartOffsetCheckpointIntervalMs,
            // TODO_MA 马中华 注释： log.retention.check.interval.ms
            // TODO_MA 马中华 注释： The frequency in milliseconds that the log cleaner checks whether any log is eligible for deletion
            retentionCheckMs = config.logCleanupIntervalMs,
            // TODO_MA 马中华 注释： log.transaction.max.timeout.ms
            maxTransactionTimeoutMs = config.transactionMaxTimeoutMs,
            // TODO_MA 马中华 注释： transactional.id.expiration.ms
