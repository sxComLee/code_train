...
abstract class ReplicaStateMachine(controllerContext: ControllerContext) extends Logging {
    ...
    def startup(): Unit = {
         ...
         *  注释： 处理每个 Partition 的每个 replica 的状态，分别进行 OnlineReplica 和 ReplicaDeletionIneligible 的状态标记
         */
        initializeReplicaState()

        info("Triggering online replica state changes")
        // TODO_MA 马中华 注释： 区分出来 在线的 replica 和 不在线的 replica
    ...
    private def initializeReplicaState(): Unit = {
        ...
        // TODO_MA 马中华 注释： 遍历所有的 Partition
        controllerContext.allPartitions.foreach { partition =>

            // TODO_MA 马中华 注释： 获取 Partition 的 replica 列表
            val replicas = controllerContext.partitionReplicaAssignment(partition)

            // TODO_MA 马中华 注释： 遍历每一个 replica
            replicas.foreach { replicaId =>

                val partitionAndReplica = PartitionAndReplica(partition, replicaId)

                // TODO_MA 马中华 注释： 如果 replica 在线，就丢进 replicaStates 集合中，标记为 OnlineReplica
                if (controllerContext.isReplicaOnline(replicaId, partition)) {
                    controllerContext.putReplicaState(partitionAndReplica, OnlineReplica)
                }
                // TODO_MA 马中华 注释： 否则，丢进 replicaStates 集合中，标记为 ReplicaDeletionIneligible
                else {
...
/**
 * // TODO_MA 马中华 注释： 此类表示副本的状态机。
 * // TODO_MA 马中华 注释： 它定义了副本可以处于的状态，并转换为将副本移动到另一个合法状态。
 * This class represents the state machine for replicas. It defines the states that a replica can be in, and
 * transitions to move the replica to another legal state. The different states that a replica can be in are -
 *
 * // TODO_MA 马中华 注释： 控制器可以在分区重新分配期间创建新副本。
 * 1. NewReplica        : The controller can create new replicas during partition reassignment. In this state, a
 *                        replica can only get become follower state change request.  Valid previous
 *                        state is NonExistentReplica
 *
 * // TODO_MA 马中华 注释： 一旦启动了一个副本并为其分区分配了一部分副本，它就处于此状态。
 ...
 *
 * // TODO_MA 马中华 注释： 当一个 replica 死亡的时候
 ...
 *
 * // TODO_MA 马中华 注释： 当一个 Replica 删除成功之后
...
class ZkReplicaStateMachine(config: KafkaConfig,
    ...
    override def handleStateChanges(replicas: Seq[PartitionAndReplica], targetState: ReplicaState): Unit = {
        if (replicas.nonEmpty) {
            try {
                 ...
                 *  注释：
                 */
                replicas.groupBy(_.replica).forKeyValue {
                    (replicaId, replicas) => doHandleStateChanges(replicaId, replicas, targetState)
                }
     ...
     *
     * // TODO_MA 马中华 注释： 将带有当前领导者和 isr 的 LeaderAndIsr 请求发送到新副本，并将分区的 UpdateMetadata 请求发送到每个实时代理
     * NonExistentReplica --> NewReplica
     * --send LeaderAndIsr request with current leader and isr to the new replica and UpdateMetadata request for the
     *   partition to every live broker
     *
     * // TODO_MA 马中华 注释： 如果需要，将新副本添加到分配的副本列表中
     ...
     *   partition to every live broker
     *
     * // TODO_MA 马中华 注释： 向副本发送 StopReplicaRequest
     * // TODO_MA 马中华 注释： 从 isr 中删除这个副本，并将 LeaderAndIsr 请求（带有新的 isr）发送到领导者副本，并将分区的 UpdateMetadata 请求发送到每个实时代理。
     ...
     * -- mark the state of the replica in the state machine
     *
     * // TODO_MA 马中华 注释： 从内存分区副本分配缓存中删除副本
    ...
    private def logSuccessfulTransition(logger: StateChangeLogger, replicaId: Int, partition: TopicPartition,
        currState: ReplicaState, targetState: ReplicaState
    ): Unit = {
