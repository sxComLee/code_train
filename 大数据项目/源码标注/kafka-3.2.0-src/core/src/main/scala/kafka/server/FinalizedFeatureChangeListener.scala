...
/**
 * // TODO_MA 马中华 注释： 通过 ZooKeeper 客户端来监听 /future znode，内部维护了一个 FinalizedFeatureCache
 * Listens to changes in the ZK feature node, via the ZK client.
 *
 * // TODO_MA 马中华 注释： 每当收到来自 ZK 的更改通知时，FinalizedFeatureCache 中的特征缓存都会异步更新为从 ZK 读取的最新特征。
 * Whenever a change notification
 * is received from ZK, the feature cache in FinalizedFeatureCache is asynchronously updated
 * to the latest features read from ZK.
 *
 * // TODO_MA 马中华 注释： 缓存更新通过单个通知处理器线程进行序列化。
 * The cache updates are serialized through a single notification processor thread.
 *
...
class FinalizedFeatureChangeListener(private val finalizedFeatureCache: FinalizedFeatureCache,
    ...
    def initOrThrow(waitOnceForCacheUpdateMs: Long): Unit = {
        ...
        // TODO_MA 马中华 注释： 一个用来维护 FinalizedFeatureCache 的工作组件 FeatureCacheUpdater
        val ensureCacheUpdateOnce = new FeatureCacheUpdater(
            // TODO_MA 马中华 注释： /future
