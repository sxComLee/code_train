 ...
 *  注释： 当前这个 KafkaController 类的主构造器 中的所有形参 其实都是这个 KafkaController 的成员变量
 *  调用构造器传递过来的 实参，其实就完成 成员变量的设置
 *  this.xx = xx;
 */
class KafkaController(val config: KafkaConfig,
    ...
    /**
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： 负责管理 Controller 和 Broker 之间的链接
     */
    ...
    /**
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： 在 KafkaController 的内部启动了一个 ControllerEventManager
     *  它是一个将 事件生产 和 消费分开来的组件。
     *  1、外部生产事件，通过 eventManager 的 put() 方法提交
     *  2、下游事件消费，通过 processor 的 process() 方法处理， 注意 Porcessor 就是 KafkaController 自己。
     */
    ...
    /**
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： Controller 监听 ZooKeeper 上 znode 状态的各种监听处理器
     *  监听处理有两大大核心类型，对应到三个不同的 Zookeeper 事件：
     *  1、ZNodeChildChangeHandler：NodeChildrenChanged
     *  2、ZNodeChangeHandler：NodeDataChanged + NodeCreated & NodeDeleted
     *  -
     *  ControllerChangeHandler controllerChangeHandler 监听 /controller znode 节点
     *  BrokerChangeHandler brokerChangeHandler 监听 /brokers/ids
     */
    ...
    def startup() = {
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 注册一个 StateChangeHandler 用来监听是否完成了和 ZooKeeper 的链接建立。
         *  如果完成了，则提交 RegisterBrokerAndReelect 事件到 ControllerEventManager 中
         *  -
         *  之后的核心逻辑，请看 ControllerEventManager 处理 RegisterBrokerAndReelect 事件
         *  这个地方注册，监听其实就是等着完成和 ZK 的链接建立
         */
        zkClient.registerStateChangeHandler(new StateChangeHandler {
            override val name: String = StateChangeHandlers.ControllerHandler

            // TODO_MA 马中华 注释： 这个方法是在 Kafka Broker 完成和 ZK 的链接建立之后的回调
            // TODO_MA 马中华 注释： 完成和 ZK 的链接建立之后： 提交 RegisterBrokerAndReelect 事件给 EventManager
            // TODO_MA 马中华 注释： 1、 完成当前 KafkaController 对应的 Broker 的注册
            // TODO_MA 马中华 注释： 2、 完成当前 KafkaController 的选举
            override def afterInitializingSession(): Unit = {

                // TODO_MA 马中华 注释： 1、找 KafkaController 的 processor() 的 RegisterBrokerAndReelect 的事件处理分支
                // TODO_MA 马中华 注释： 2、直接搜： case RegisterBrokerAndReelect
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 往 ControllerEventManager 中提交 Startup 事件
         *  你可以按照这样的公式去记忆：
         *  1、提交一个叫做  XXXX 的事件
         *  2、调用 processXXXX() 来完成处理
         *  基本上适用！
         */
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： ControllerEventManager 启动
         *  eventManager 是完成 上游事件生产 和 下游事件消费的 解耦
         *  它内部怎么完成事件的管理和调度？
         *  1、维护了一个队列，其实 put(Event) 其实就是提交到了 队列中
         *  2、维护了一个队列消费线程， eventManager.start() 的核心逻辑就是启动这个消费线程
         *        从队列中获取事件
         *        然后调用 processor.process(Event) 完成处理
         */
        eventManager.start()
    }
    ...
    /**
     * // TODO_MA 马中华 注释： Zookeeper 领导选举人在选举当前代理作为新控制器时调用此回调。它对成为控制器状态更改执行以下操作 -
     * This callback is invoked by the zookeeper leader elector on electing the current broker as the new controller.
     * It does the following things on the become-controller state change -
     *
     * // TODO_MA 马中华 注释： 初始化控制器的上下文对象，该对象包含所有现有分区的当前主题、实时代理和领导者的缓存对象。
     * 1. Initializes the controller's context object that holds cache objects for current topics, live brokers and
     *    leaders for all existing partitions.
     *
     * // TODO_MA 马中华 注释： 启动控制器的通道管理器
     * 2. Starts the controller's channel manager
     *
     * // TODO_MA 马中华 注释： 启动副本状态机
     * 3. Starts the replica state machine
     *
     * // TODO_MA 马中华 注释： 启动分区状态机
     * 4. Starts the partition state machine
     *
     * // TODO_MA 马中华 注释： 如果它在成为控制器时遇到任何意外的异常错误，它将作为当前控制器辞职。
     * // TODO_MA 马中华 注释： 这确保将触发另一个控制器选举，并且始终有一个主动服务的控制器
    ...
    private def onControllerFailover(): Unit = {
        ...
        // TODO_MA 马中华 注释： 首先注册： 关于 broker 和 topic 的监听器，其实就是回调处理器
        val childChangeHandlers = Seq(brokerChangeHandler, topicChangeHandler, topicDeletionHandler, logDirEventNotificationHandler,
            isrChangeNotificationHandler)
        childChangeHandlers.foreach(zkClient.registerZNodeChildChangeHandler)

        // TODO_MA 马中华 注释： 再次注册： 关于 replica 和 partition 的监听器
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 启动 Replica 的 StateMachine
         */
        replicaStateMachine.startup()

        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 启动 Partition 的 StateMachine
         */
        partitionStateMachine.startup()
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： Partition Leader 分布负载均衡检查任务
         */
    ...
    /**
     * // TODO_MA 马中华 注释： 当当前代理辞去控制器的职务时，zookeeper 领导选举人会调用此回调。
     * This callback is invoked by the zookeeper leader elector when the current broker resigns as the controller.
     *
     * // TODO_MA 马中华 注释： 这是清理内部控制器数据结构所必需的
     * This is required to clean up internal controller data structures
     */
    private def onControllerResignation(): Unit = {
        ...
        // TODO_MA 马中华 注释： 注销各种 zookeeper 的监听回调，减小负载
        ...
        // TODO_MA 马中华 注释： kafkaScheduler 关闭
        ...
        // TODO_MA 马中华 注释：  tokenCleanScheduler 关闭
        ...
        // TODO_MA 马中华 注释： partitionStateMachine 关闭
        ...
        // TODO_MA 马中华 注释： replicaStateMachine 关闭
        replicaStateMachine.shutdown()
        zkClient.unregisterZNodeChildChangeHandler(brokerChangeHandler.path)

        // TODO_MA 马中华 注释： controllerChannelManager 关闭
        controllerChannelManager.shutdown()

        // TODO_MA 马中华 注释： 维护了 Controller 内部的各种数据结构，存储了集群元数据
    ...
    private def initializeControllerContext(): Unit = {
        ...
        // TODO_MA 马中华 注释： 首先，查询得到
        // TODO_MA 马中华 注释： 1、查询 /kafka/brokers/ids/ 下的子节点列表
        // TODO_MA 马中华 注释： 2、查询每个 /kafka/brokers/ids/brokerid 节点的值
        ...
        // TODO_MA 马中华 注释： 注册监听器
        registerPartitionModificationsHandlers(controllerContext.allTopics.toSeq)

        // TODO_MA 马中华 注释： 查询 /brokers/topics/topic 节点的值
        ...
        // TODO_MA 马中华 注释： 注册 Broker 相关的监听器
        registerBrokerModificationsHandler(controllerContext.liveOrShuttingDownBrokerIds)

        // update the leader and isr cache for all existing partitions from Zookeeper
        // TODO_MA 马中华 注释： 修改 Partition 的 Leader 和 ISR 信息
        updateLeaderAndIsrCache()

        // start the channel manager
        // TODO_MA 马中华 注释： ControllerChannelManager 启动
    ...
    private def fetchTopicDeletionsInProgress(): (Set[String], Set[String]) = {
        ...
        // TODO_MA 马中华 注释： 获取 /admin/delete_topics 节点下的待删除的 topic
        val topicsToBeDeleted = zkClient.getTopicDeletions.toSet

        // TODO_MA 马中华 注释： 从所有 Topic 中过滤出去要删除的 topic
        val topicsWithOfflineReplicas = controllerContext.allTopics.filter {
            topic => {
    ...
    private def updateLeaderAndIsrCache(partitions: Seq[TopicPartition] = controllerContext.allPartitions.toSeq): Unit = {
        ...
        // TODO_MA 马中华 注释： 从 ZK 获取每个 Partition 的 state
    ...
    private def maybeResign(): Unit = {
        ...
        // TODO_MA 马中华 注释： 第一次判断
        // TODO_MA 马中华 注释： 检查自己是否已经是 active Controller
        val wasActiveBeforeChange = isActive

        // TODO_MA 马中华 注释： 1、注册监听回调处理器
        // TODO_MA 马中华 注释： 2、检查 Controller znode 节点 /controller 是否已经存在
        zkClient.registerZNodeChangeHandlerAndCheckExistence(controllerChangeHandler)

        // TODO_MA 马中华 注释： 查询 /controller 节点的值
        activeControllerId = zkClient.getControllerId.getOrElse(-1)

        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 检查之前，自己是 active，检查之后不是了，则关闭各种工作组件
         */
        // TODO_MA 马中华 注释： 第二次判断
        if (wasActiveBeforeChange && !isActive) {

            // TODO_MA 马中华 注释： 关闭掉 KafkaController 中的各种服务组件
            onControllerResignation()
        }
    }
    ...
    /**
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： KafkaController 进行选举
     */
    private def elect(): Unit = {

        // TODO_MA 马中华 注释： 先获取 controller 节点的值
        // TODO_MA 马中华 注释： {"version":1,"brokerid":6,"timestamp":"1656407124851"}
        activeControllerId = zkClient.getControllerId.getOrElse(-1)
        /*
        ...
        try {
            /**
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 创建 /kafka/controller 节点成为 active Controller 也就是 Leader Controller
             */
    ...
    private def partitionOnFeatureCompatibility(brokersAndEpochs: Map[Broker, Long]): (Map[Broker, Long], Map[Broker, Long]) = {
        ...
        brokersAndEpochs.partition {
            case (broker, _) =>
                !config.isFeatureVersioningSupported || !featureCache.get.exists(
                    latestFinalizedFeatures => BrokerFeatures.hasIncompatibleFeatures(broker.features,
    ...
    /**
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： 内部代码逻辑，有点杂，总之就是计算出来 那些节点是新上线的，那些节点是刚才下线的。
     *  接下来就调用其他的服务组件来完成 Broker 上线和 Broker 下线需要干的工作
     */
    private def processBrokerChange(): Unit = {

        // TODO_MA 马中华 注释： 当前 Controller 如果不是 active，直接返回
        if (!isActive) return

        // TODO_MA 马中华 注释： 从 ZK 获取集群中的所有的 broker 的注册信息
        val curBrokerAndEpochs = zkClient.getAllBrokerAndEpochsInCluster

        // TODO_MA 马中华 注释： 获取到了上线的所有的 Broker 的 ID
        val curBrokerIdAndEpochs = curBrokerAndEpochs map { case (broker, epoch) => (broker.id, epoch) }

        // TODO_MA 马中华 注释： 此时 整个集群的所有的活着的 Broker
        val curBrokerIds = curBrokerIdAndEpochs.keySet

        // TODO_MA 马中华 注释： 原来的
        val liveOrShuttingDownBrokerIds = controllerContext.liveOrShuttingDownBrokerIds

        // TODO_MA 马中华 注释： 新上线的 Broker
        val newBrokerIds = curBrokerIds.diff(liveOrShuttingDownBrokerIds)

        // TODO_MA 马中华 注释： 死掉的 Broker
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 从 ControllerChannelManager 更新 Broker 的相关信息
         */
    ...
    private def validateFeatureUpdate(update: UpdateFeaturesRequestData.FeatureUpdateKey,
        existingVersionRange: Option[FinalizedVersionRange]
    ...
    private def processFeatureUpdates(request: UpdateFeaturesRequest,
        callback: UpdateFeaturesCallback
    ): Unit = {
        if (isActive) {
    ...
    def listPartitionReassignments(partitions: Option[Set[TopicPartition]],
        callback: ListReassignmentsCallback
    ): Unit = {
    ...
    private def processReelect(): Unit = {
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 第一件事： 检查 znode（/controller） 节点是否存在，如果存在，并且不是自己的，则关闭各种工作组件
         *  1、我的集群： /kafka272/controller
         *  2、默认情况： /kafka/controller   Controller 的注册节点
         */
        maybeResign()

        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 第二件事： KafkaController 执行选举
         */
        elect()
    }
    ...
    private def processRegisterBrokerAndReelect(): Unit = {
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： Broker 注册, 确保机制。 Broker 一上线之前已经注册过了
         */
    ...
    /**
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： 根据 ControllerEvent 的具体类型，来决定
     *  如果想让大家把这段代码，改成 扩展性 和 可维护性 极强的 java 写法？
     *
     *  interface Handler{
     *      void handle();
     *  }
     *
     *  class MockEventHandler implements Handler{
     *      void handle(){
     *          event.process()
     *      }
     *  }
    ...
    override def process(event: ControllerEvent): Unit = {
        try {
            event match {
                ...
                /**
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 有 Broker 发生变化： Broker 上线 或者 Broker 下线
                 */
                case BrokerChange => processBrokerChange()
                ...
                /**
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： KafkaController 的选举监听回调的处理逻辑
                 *  当 Controller 被选举出来，所有的节点，都会处理 一个 ControllerChange 这个事件
                 *  1、如果我是 Leader Controller：
                 *  2、如果我不是 Leader Controller：
                 */
                ...
                /**
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 这是 KafkaController 启动启动来的第一件事
                 */
...
class BrokerChangeHandler(eventManager: ControllerEventManager) extends ZNodeChildChangeHandler {
    ...
    // TODO_MA 马中华 注释： /kafka/brokers/ids
    override val path: String = BrokerIdsZNode.path

    // TODO_MA 马中华 注释： 处理 /kafka/brokers/ids 下的子节点的增多 或者 减少
    // TODO_MA 马中华 注释： /kafka/brokers/ids/brokerid 这种类型的节点增加代表有 Broker 上线
    // TODO_MA 马中华 注释： /kafka/brokers/ids/brokerid 这种类型的节点减少代表有 Broker 下线
    // TODO_MA 马中华 注释： 如果响应了，也有可能个数不变！ 上线了 N 个节点，同时也下线了 N 个节点
...
class TopicDeletionHandler(eventManager: ControllerEventManager) extends ZNodeChildChangeHandler {
    ...
    // TODO_MA 马中华 注释： 提交 TopicDeletion 事件给 ControllerEventManager
    // TODO_MA 马中华 注释： 由 ControllerEventManager 中的 ControllerEventProcessor 来完成处理
    // TODO_MA 马中华 注释： ControllerEventProcessor 就是 KafkaController
    // TODO_MA 马中华 注释： KafkaController 的 process() 完成事件的处理； 根据事件的类型，调用不同的业务方法
...
class ControllerChangeHandler(eventManager: ControllerEventManager) extends ZNodeChangeHandler {
    ...
    // TODO_MA 马中华 注释： /controller
    override val path: String = ControllerZNode.path

    // TODO_MA 马中华 注释： Controller 发生改变，有 Controller 角色创建 /controller 节点成功，提交 ControllerChange 事件处理
    override def handleCreation(): Unit = eventManager.put(ControllerChange)

    // TODO_MA 马中华 注释： 当 /controller znode 节点被删除的时候，触发重新选举
    override def handleDeletion(): Unit = eventManager.put(Reelect)

    // TODO_MA 马中华 注释： /controller znode 节点的数据发生改变，处理 ControllerChange 事件
