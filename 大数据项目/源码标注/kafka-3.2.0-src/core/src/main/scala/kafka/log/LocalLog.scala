...
class LocalLog(@volatile private var _dir: File,
    ...
    private[log] def append(lastOffset: Long, largestTimestamp: Long, shallowOffsetOfMaxTimestamp: Long, records: MemoryRecords): Unit = {
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 将数据写到 Active Segment
         *  UnifiedLog 内部为了一个组件：LogSegments
         *  LogSegments 内部通过一个 ConcurrentSkipListMap 跳表来维护这个分区目录下的所有的 LogSegment
         *  -
         *  1、写 log ==> 通过 FileChannel 写的
         *  2、写 index ==> 通过 MappedByteBuffer 来实现
         */
    ...
    private def collectAbortedTransactions(startOffset: Long, upperBoundOffset: Long,
        ...
        accumulator: List[AbortedTxn] => Unit
    ): Unit = {
    ...
    private[log] def roll(expectedNextOffset: Option[Long] = None): LogSegment = {
        maybeHandleIOException(s"Error while rolling log segment for $topicPartition in dir ${dir.getParent}") {
            ...
            /**
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 创建一个新的 LogSegment 用来完成数据的写入
             */
            ...
            // TODO_MA 马中华 注释： 更新 LEO
            updateLogEndOffset(nextOffsetMetadata.messageOffset)

            info(s"Rolled new log segment at offset $newOffset in ${time.hiResClockMs() - start} ms.")

            // TODO_MA 马中华 注释： 返回 LogSegment
            newSegment
        }
    }
