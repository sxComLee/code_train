...
class UnifiedLog(@volatile var logStartOffset: Long,
    ...
    locally {
        ...
        // TODO_MA 马中华 注释： 创建一个 partition.metadata 用来存储分区的元数据信息
        initializePartitionMetadata()

        // TODO_MA 马中华 注释： 更新 HW 和 recovery point
        updateLogStartOffset(logStartOffset)

        // TODO_MA 马中华 注释： 如果有需要，则更新 Unstable offset
        maybeIncrementFirstUnstableOffset()

        // TODO_MA 马中华 注释： 从分区元数据存储文件 partition.metadata 中读取到的 Topic ID
        initializeTopicId()
    }
    ...
    private def initializePartitionMetadata(): Unit = lock synchronized {
        // TODO_MA 马中华 注释： partition.metadata
    ...
    private def updateLogStartOffset(offset: Long): Unit = {
        ...
        // TODO_MA 马中华 注释： 更新 HW 为新的 offset
        if (highWatermark < offset) {
            updateHighWatermark(offset)
        }

        // TODO_MA 马中华 注释： 更新 recovery point 为新的 offset
    ...
    private def append(records: MemoryRecords,
        ...
        // TODO_MA 马中华 注释： 检查是否需要 flush 元数据存储文件
        // TODO_MA 马中华 注释： partition.metadata
        maybeFlushMetadataFile()
    ...
    private def maybeRoll(messagesSize: Int, appendInfo: LogAppendInfo): LogSegment = lock synchronized {
        ...
        // TODO_MA 马中华 注释： 获取该 partition replica 的 active Segment
...
object UnifiedLog extends Logging {
    ...
    def apply(dir: File,
        ...
        // TODO_MA 马中华 注释： 通过解析 文件夹名称得到 分区定义
        val topicPartition = UnifiedLog.parseTopicPartitionName(dir)

        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 构造一个 LogSegments 用来管理每个日志文件对应的 LogSegment
         */
        val segments = new LogSegments(topicPartition)

        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 创建 leader-epoch-checkpoint 文件用来存储 leader Epoch
         */
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 维护从 ProducerIds 到有关最后附加条目的元数据的映射（例如，纪元、序列号、最后偏移量等）
         *  通俗的说，Kafka 想实现幂等，就是靠 ProducerStateManager 维护 sequence number 序列化实现的
         */
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 构建 LogSegment 管理日志文件, 每个 LogSegment 对象都会加入到 LogSegments 内维护的一个跳表中进行管理
         *  内部会构建 LogSegment 然后加入到 LogSegments segments 对象中执行管理， 同时也得到每个 LogSegment 的 offset
         *  注意会按照 LogSegment 的大小排序，最大的 offset 对应的 LogSegment 其实就是最后一个 LogSegment 就是 Active LogSegment
         */
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 构建 LocalLog 用来管理这个 LogDir 除了 LogSegments 之外的信息
         *  负责管理本地的日志追加写
         */
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 最后构造一个 UnifiedLog 实例用来维护一个 LogDir
         */
    ...
    def maybeCreateLeaderEpochCache(dir: File,
        ...
        // TODO_MA 马中华 注释： leader-epoch-checkpoint
