...
class ZooKeeperClient(connectString: String,
     ...
     *  注释： 处理 NodeDeleted，NodeCreated，NodeDataChanged 事件的 Handler
     */
    private val zNodeChangeHandlers = new ConcurrentHashMap[String, ZNodeChangeHandler]().asScala

    /**
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： 处理 NodeChildrenChanged 事件的 Handler
     */
    ...
    /**
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： 处理与 ZooKeeper 的链接状态的 Handler
     */
    ...
    /**
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： ZooKeeper 的 原生 API
     *  注意： ZooKeeperClientWatcher，就是监听器
     */
    @volatile
    private var zooKeeper = new ZooKeeper(connectString, sessionTimeoutMs, ZooKeeperClientWatcher, clientConfig)
     ...
     * // TODO_MA 马中华 注释： 创建节点，判断节点存在与否，修改节点的值的，等等，都是封装成一个 XXXRequest
     */
    def handleRequests[Req <: AsyncRequest](requests: Seq[Req]): Seq[Req#Response] = {
        if (requests.isEmpty)
            ...
            // TODO_MA 马中华 注释： // 遍历每个请求，进行发送，并且构造响应结果集
            requests.foreach { request =>
                ...
                try {
                    inReadLock(initializationLock) {
                        ...
                        /**
                         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                         *  注释： 具体发送请求的地方，内部是通过异步的方式发送请求给 ZooKeeper，然后通过回调解析响应加入到响应集合中
                         */
    ...
    private[zookeeper] def send[Req <: AsyncRequest](request: Req)(processResponse: Req#Response => Unit): Unit = {
        ...
        (request: AsyncRequest) match {
            // TODO_MA 马中华 注释： zookeeper.exists()
            ...
            case CreateRequest(path, data, acl, createMode, ctx) =>
                ...
                // TODO_MA 马中华 注释： ZooKeepeer 原生实现
    ...
    private[zookeeper] def scheduleReinitialize(name: String, message: String, delayMs: Long): Unit = {
        reinitializeScheduler.schedule(name, () => {
            info(message)
            // TODO_MA 马中华 注释： 重新初始化，建立和 ZK 的链接
            reinitialize()
    ...
    private[zookeeper] object ZooKeeperClientWatcher extends Watcher {
        ...
        // TODO_MA 马中华 注释： Watcher 的回调方法
        override def process(event: WatchedEvent): Unit = {
            debug(s"Received event: $event")

            // TODO_MA 马中华 注释： 根据路径来处理
...
/**
 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
 *  注释： 处理与 ZooKeeper 的链接相关事件处理的 Handler
 */
...
/**
 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
 *  注释： ZooKeeper 的 NodeDataChanged 和 NodeCreated、NodeDeleted 事件的处理器
 */
...
/**
 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
 *  注释： ZooKeeper 的 NodeChildrenChanged 事件的处理器
 */
