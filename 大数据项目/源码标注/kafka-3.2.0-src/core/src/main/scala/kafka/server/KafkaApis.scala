...
class KafkaApis(val requestChannel: RequestChannel,
    ...
    override def handle(request: RequestChannel.Request, requestLocal: RequestLocal): Unit = {
        try {
            ...
            /**
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 根据请求的具体的类型，来调用对应的业务
             */
            request.header.apiKey match {

                // TODO_MA 马中华 注释： 生产（KafkaProducer 发送过来的发送数据的请求）
                case ApiKeys.PRODUCE => handleProduceRequest(request, requestLocal)

                // TODO_MA 马中华 注释： 消费（KafkaConsumer 发送过来的拉取数据的请求）
                case ApiKeys.FETCH => handleFetchRequest(request)

                case ApiKeys.LIST_OFFSETS => handleListOffsetRequest(request)
                case ApiKeys.METADATA => handleTopicMetadataRequest(request)

                // TODO_MA 马中华 注释： Replica 状态切换请求
                // TODO_MA 马中华 注释： 这种类型的请求是 Leader KafkaController 给所有的 Broker 发送的
                // TODO_MA 马中华 注释： 用来完成 Replica 的状态维护
    ...
    // TODO_MA 马中华 注释： 做了一些简单处理之后，将处理转交给 ReplicaManager ，和 Produce 类型请求一样
    def handleLeaderAndIsrRequest(request: RequestChannel.Request): Unit = {
        ...
        if (isBrokerEpochStale(zkSupport, leaderAndIsrRequest.brokerEpoch)) {
            ...
            /**
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： LeaderAndIsrRequest 请求的 内部实现
             *  PRODUCE 请求:  replicaManager.appendRecords()
             *  LeaderAndIsrRequest 请求： replicaManager.becomeLeaderOrFollower()
             */
            val response = replicaManager.becomeLeaderOrFollower(correlationId, leaderAndIsrRequest,

                // TODO_MA 马中华 注释： Replia 的状态发生变更的回调
    ...
    def handleProduceRequest(request: RequestChannel.Request, requestLocal: RequestLocal): Unit = {
        ...
        // TODO_MA 马中华 注释： 未校验通过，生成的响应集合
        val unauthorizedTopicResponses = mutable.Map[TopicPartition, PartitionResponse]()
        val nonExistingTopicResponses = mutable.Map[TopicPartition, PartitionResponse]()
        val invalidRequestResponses = mutable.Map[TopicPartition, PartitionResponse]()

        // TODO_MA 马中华 注释： 校验通过的请求数据集合， 按照分区进行分类管理
        ...
        // TODO_MA 马中华 注释： 定义了一个发送响应给客户端的方法
        ...
        // TODO_MA 马中华 注释： 如果 PRODUCE 类生产请求，并没有什么需要处理的数据
        if (authorizedRequestInfo.isEmpty)
            ...
            /**
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 调用 ReplicaManager 去写数据到 Partition 的多个副本
             */
    ...
    private def metadataResponseTopic(error: Errors,
        ...
        topicId: Uuid,
        isInternal: Boolean,
        partitionData: util.List[MetadataResponsePartition]
    ...
    private def fetchOffsets(groupId: String, isAllPartitions: Boolean, requireStable: Boolean,
        partitions: util.List[TopicPartition], context: RequestContext
    ...
    private def updateRecordConversionStats(request: RequestChannel.Request,
        tp: TopicPartition,
        conversionStats: RecordConversionStats
    ): Unit = {
