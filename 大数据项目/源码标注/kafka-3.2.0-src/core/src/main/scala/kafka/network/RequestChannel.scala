 ...
 *  注释： 一个用来管理接受到客户端的所有请求数据的整体抽象，内部核心是维护一个队列，然后由下游处理线程来获取消费
 *  class RequestChannel  伴生类
 *  object RequestChannel  伴生对象（都是静态的）
 *  伴生关系
 */
class RequestChannel(val queueSize: Int,
    ...
    /**
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： RequestChannel 内部维护了一个 BaseRequest 请求队列
     */
    private val requestQueue = new ArrayBlockingQueue[BaseRequest](queueSize)

    /**
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： 同时也管理一堆 Processor
     */
    ...
    private[network] def sendResponse(response: RequestChannel.Response): Unit = {
        ...
        // TODO_MA 马中华 注释： 找到该 Response 对应的 Processor
        ...
        /**
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 将处理之后得到的响应结果，注入到 Processor 中的 responseQueue 队列中
         */
        if (processor != null) {
    ...
    /**
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： 下述两个方法，是下游的 KafkaRequestHandle 线程调用的
     */

    // TODO_MA 马中华 注释： KafkaRequestHandler 线程会调用该方法来从队列获取 Request，阻塞方法
    /** Get the next request or block until specified time has elapsed */
    def receiveRequest(timeout: Long): RequestChannel.BaseRequest =
        requestQueue.poll(timeout, TimeUnit.MILLISECONDS)

    /** Get the next request or block until there is one */
    // TODO_MA 马中华 注释： KafkaRequestHandler 线程会调用该方法来从队列获取 Request，非阻塞方法
