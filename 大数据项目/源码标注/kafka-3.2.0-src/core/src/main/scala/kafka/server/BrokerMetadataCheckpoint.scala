...
object BrokerMetadataCheckpoint extends Logging {
    ...
    def getBrokerMetadataAndOfflineDirs(
         ...
         *  注释： 遍历 Broker 的日志存储目录
         */
        for (logDir <- logDirs) {

            // TODO_MA 马中华 注释： 先检查日志目录中是否存在 checkpint 文件 meta.properties
            ...
            try {
                ...
                /**
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 去读取日志目录下的 meta.properties 文件
                 */
        ...
        // TODO_MA 马中华 注释： 如果读取 meta.properties 没有数据，则构建一个空的 元数据 返回
        if (brokerMetadataMap.isEmpty) {
            (new RawMetaProperties(), offlineDirs)
        } else {

            // TODO_MA 马中华 注释： 去重，检查是否存在冲突的元数据
...
class BrokerMetadataCheckpoint(val file: File) extends Logging {
    ...
    /**
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： 进行 元数据 checkpoint 的方法，用来写 meta.properties 文件
     */
    def write(properties: Properties): Unit = {
        lock synchronized {
            try {
                // TODO_MA 马中华 注释： 先写临时文件
    ...
    /**
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： 读取 元数据 checkpoint 文件： meta.properties 的内容
     */
