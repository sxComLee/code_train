 ...
 * LogSement 代表一个日志片段： baseoffset.log + baseoffset.index 组成
 *      FileRecords
 *          FileChannel
 */
@nonthreadsafe
class LogSegment private[log](
    // TODO_MA 马中华 注释： FileRecords 内部维护了一个 FileChannel 帮助我们使用 零拷贝的技术完成 文件的写操作
    val log: FileRecords,
    ...
    def append(largestOffset: Long,
        ...
        if (records.sizeInBytes > 0) {
            ...
            /**
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 将 MemoryRecords 写到 Log 中
             *  log = FileRecords
             *  FileRecords 它维护了一个 FileChannel , 用来向 baseoffset.log 文件写数据
             */
...
object LogSegment {
    ...
    def open(dir: File, baseOffset: Long, config: LogConfig, time: Time, fileAlreadyExists: Boolean = false,
        ...
        // TODO_MA 马中华 注释： 创建一个 LogSegment 实例
        new LogSegment(
