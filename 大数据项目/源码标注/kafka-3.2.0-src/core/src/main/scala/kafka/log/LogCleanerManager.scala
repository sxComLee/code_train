...
) extends KafkaException(message, cause)
...
private[log] class LogCleanerManager(val logDirs: Seq[File],
    ...
    // TODO_MA 马中华 注释： 用来维护 data 数据目录 和 cleaner-offset-checkpoint 之间的映射关系
    ...
    // TODO_MA 马中华 注释： 用来记录正在执行清理的 TopicPartition 的压缩状态
    ...
    def grabFilthiestCompactedLog(time: Time, preCleanStats: PreCleanStats = new PreCleanStats()): Option[LogToClean] = {
        inLock(lock) {
            ...
            // TODO_MA 马中华 注释： 处理得到需要进行日志压缩的 Log 集合
            ...
            // TODO_MA 马中华 注释： 如果有需要执行压缩的 Log, 则登记一下，登记到 inProgress 中
    ...
    def updateCheckpoints(dataDir: File,
        ...
        partitionToRemove: Option[TopicPartition] = None
    ): Unit = {
        inLock(lock) {
